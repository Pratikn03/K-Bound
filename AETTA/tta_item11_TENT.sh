...
TGTS="${TGTS_OVERRIDE:-gaussian_noise-5
    shot_noise-5
    impulse_noise-5
    defocus_blur-5
    glass_blur-5
...
mkdir -p raw_logs # save console outputs here
...
wait_n() {
  ...
  background=($(jobs -p))
  local default_num_jobs=${AETTA_MAX_JOBS:-2}  # one Apple MPS device: avoid 15-way oversubscription
