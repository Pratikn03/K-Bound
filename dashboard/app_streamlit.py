...
def _render_app():  # pragma: no cover - UI code
    ...
    if option == "All domains":
        ...
        harness_path = EXPERIMENTS_DIR / "fusion" / "attention_fusion" / "harness_metrics.json"
        if harness_path.exists():
            harness = json.loads(harness_path.read_text())
            baseline = harness.get("baseline", {}).get("summary", {}).get("metrics", {})
            if baseline:
                st.subheader("Attention Fusion (Baseline) Summary")
                df = pd.DataFrame(
                    [{"Metric": k, "Mean": v.get("mean"), "Std": v.get("std")} for k, v in baseline.items()]
                )
                st.dataframe(df)
        plots_dir = EXPERIMENTS_DIR / "fusion" / "attention_fusion" / "plots"
        heatmap_path = plots_dir / "attention_heatmap.png"
        contrib_path = plots_dir / "domain_contributions.png"
        if heatmap_path.exists():
            st.image(str(heatmap_path), caption="Attention Heatmap (Average)")
        if contrib_path.exists():
            st.image(str(contrib_path), caption="Domain Contributions")
