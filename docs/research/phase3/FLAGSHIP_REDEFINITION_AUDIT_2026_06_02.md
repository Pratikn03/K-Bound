...
| `research_lock/D16_NATURAL_DEGRADATION_SELECTOR_PROTOCOL_v1.yaml` | D16 natural-degradation headroom is separate from Gate E; opened smoke category is excluded from confirmation. |
| `research_lock/DECISIONS_v1.md` | D12 separates strict flagship readiness from bounded v3; D13 separates positive-transfer track; D14 closes clean Gate E by proof; D16 adds natural-degradation headroom without converting it to Gate E. |
| `elara_master_c/audits/confirmatory_statistics_report.json` | Current strict Gate E false, D13 official fail, D16 official fail, bounded v3 true. |
...
| `experiments/fusion/realiad_d3_headroom_audit_result.json` | Real-IAD D3 D16 official attempt is `OFFICIAL_FAIL`: stress endpoint CI crosses zero. |
...
| D16 natural degradation/headroom | OFFICIAL FAIL | Real-IAD D3 all-test delta vs CW is positive (+0.0624, CI [+0.0347,+0.0896]), but the primary stress-subset delta is +0.0351 with CI [-0.0276,+0.0980]. |
...
| Count D16 all-test Real-IAD D3 win as Gate S | No | No | D16 primary endpoint is the prelocked stress subset. All-test positivity is supportive but not a gate pass. |
...
D16 has now executed this idea on Real-IAD D3. It did not pass the primary
stress endpoint, so a future natural-degradation claim needs either a stronger
prelocked method or a new fresh holdout. The positive all-test result can be
reported only as supportive evidence.
