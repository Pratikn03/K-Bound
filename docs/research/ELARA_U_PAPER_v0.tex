...
Finally, on a \emph{sealed} one-shot holdout of $74$ untouched tasks under a
different feature extractor (ViT/RoBERTa; protocol frozen before scoring), the
stacking advantage replicates ($+0.016$ vs auto-select, CI $[+0.004,+0.032]$).
\end{abstract}
...
\label{sec:strength}
We harden the main result against six reviewer concerns. \textbf{(i) Stacker
...
were frozen (protocol D24) \emph{before} scoring $74$ untouched ADBench tasks under
a \emph{different feature extractor} than development (ViT/RoBERTa vs.\
ResNet18/BERT). On this one-shot sealed holdout the stack still beats auto-select
...
scoring $74$ untouched ADBench tasks under a different feature extractor (ViT/RoBERTa)
