...
where modalities fail \emph{independently} (Real-IAD-D3; pre-registered D23),
reliability routing decisively wins --- failing the deployment-best modality at
test, a drift-aware gate beats equal-weight fusion ($+0.22$), stale selection
($+0.33$), and validation-only fusion ($+0.20$; all CIs exclude zero) --- while
inert when nothing fails. A sixth family (time-series, NAB) shows the stacking
advantage is marginal there ($+0.008$, n.s.), bounding the positive to
feature-vector families. The precise claim: \emph{reliability routing helps iff
modalities can fail independently; otherwise plain stacking suffices}.
...
average \emph{loses} to selection, so the gain is learned combination.}
...
\begin{table}[t]\centering
\caption{D23 multimodal reliability (9 Real-IAD-D3 categories, RGB/PS/XYZ). Mean
AUROC by regime; hypotheses are paired bootstrap over categories in the
modality-failure regime.}
\label{tab:d23}\footnotesize
\begin{tabular}{lcc}
\toprule
\textbf{Method} & clean & modality-failure \\
\midrule
Equal-weight (CW) & 0.745 & 0.494 \\
Stale auto-select & 0.939 & 0.381 \\
Validation-only fusion & 0.901 & 0.517 \\
\textbf{Reliability gate} & \textbf{0.901} & \textbf{0.715} \\
\midrule
\multicolumn{3}{p{0.92\linewidth}}{H1 vs equal-weight $+0.221$ $[0.090,0.398]$; H2 vs stale-select $+0.334$ $[0.177,0.487]$; H3 vs validation-only fusion $+0.198$ $[0.090,0.305]$ --- all 95\% CIs exclude zero.}\\
\bottomrule
\end{tabular}
\end{table}
