...
drift-aware router is \emph{significantly worse} than plain stacking ($-0.050$
AUROC, 95\% CI $[-0.103,-0.013]$) and only $+0.008$ over auto-select (not
significant). Combined with the i.i.d.\ ablation, this refutes reliability-aware
routing even under genuine shift.
...
is not relitigated here. We note that while \systemu{} significantly improves the mean rank on cyber, tabular, and image-OOD families, the rank-gain bootstrap interval on text tasks is directionally positive but does not exclude zero, and fraud is underpowered with $n=1$ in the leave-family-out cross-validation. Furthermore, beyond synthetic degradation we ran a sealed natural temporal-shift benchmark (D22, Results): there too plain stacking wins and the drift-aware router is significantly worse, so the reliability/drift machinery does not help even under natural shift; native OOD and industrial sensor-drift families remain future work. Finally, full calibration metrics (Brier score and NLL) and a fitted calibration layer are pending.
