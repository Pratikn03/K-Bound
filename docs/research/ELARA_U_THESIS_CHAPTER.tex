\documentclass[11pt,titlepage]{article}
\usepackage[margin=1.0in]{geometry}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{amsmath,amssymb,amsthm}
\usepackage{booktabs}
\usepackage{array}
\usepackage{longtable}
\usepackage{graphicx}
\graphicspath{{figures/}}
\usepackage[hypertexnames=false]{hyperref}
\usepackage{xcolor}
\usepackage{url}
\usepackage{float}
\usepackage{algorithm}
\usepackage{algpseudocode}
\hypersetup{colorlinks=true,linkcolor=darkgray,citecolor=blue,urlcolor=blue}
\emergencystretch=2em

\newcommand{\systemu}{ELARA-U}
\newcommand{\system}{ELARA}
\newcommand{\method}{RGA}
\newcommand{\methodplus}{RGA+}
\newcommand{\gateu}{Gate U}
\newcommand{\relg}{\textsc{RelGate}}
\newcolumntype{L}[1]{>{\raggedright\arraybackslash}p{#1}}

% Theorem-like environments
\ifcsname c@theorem\endcsname\else\newtheorem{theorem}{Theorem}\fi
\ifcsname c@corollary\endcsname\else\newtheorem{corollary}{Corollary}\fi
\ifcsname c@proposition\endcsname\else\newtheorem{proposition}{Proposition}\fi
\ifcsname c@lemma\endcsname\else\newtheorem{lemma}{Lemma}\fi
\ifcsname c@certificate\endcsname\else\newtheorem{certificate}{Certificate}\fi
\ifcsname c@diagnostic\endcsname\else\newtheorem{diagnostic}{Diagnostic}\fi
\ifcsname c@slack\endcsname\else\newtheorem{slack}{Proposition}\fi

\begin{document}

\begin{titlepage}
\centering
\vspace*{1.7cm}
{\huge\bfseries ELARA-U: A Cross-Domain Anomaly Meta-Routing Benchmark\\ ---Stacking Beats Selection; Reliability Routing Helps Only Under Independent Modality Failure\par}
\vspace{1.4cm}
{\large\bfseries Pratik Niroula}\par
\vspace{0.4cm}
{\large Independent Researcher}\par
\vspace{1.8cm}
{\Large\itshape Expanded Thesis Chapter and Research Monograph\par}
\vspace{1.5cm}
{\large \today\par}
\vspace{1.6cm}
\begin{minipage}{0.86\linewidth}
\small
\textbf{Thesis statement.} The central problem in universal anomaly detection is
not only inventing a stronger detector, but preventing the wrong detector from
being trusted under domain shift. \systemu{} studies this as a meta-routing
problem: given a heterogeneous detector zoo and a validation slice, decide when
to select, fuse, stack, or fall back without using test labels. On the clean i.i.d.\ archive of
$123$ tasks from $5$ families, \systemu{} Stack (per-task rank-normalized
stacking) reaches mean rank $2.53$ versus $3.38$ for validation auto-selection
($+0.036$ AUROC, paired-bootstrap CI $[+0.023,+0.050]$, excluding zero; $86/123$
wins) and even exceeds the best-single-detector oracle ($0.784$ vs $0.756$). However,
controlled ablations across four regimes (Holm-corrected) and a \emph{sealed natural
temporal-shift} benchmark (D22) resolve RQ3 negatively: reliability/drift features add
no significant value in any regime, and a reliability gate on the stack is significantly
\emph{worse} ($-0.037$ AUROC, $p_{\mathrm{Holm}}{<}10^{-8}$); under natural shift the
drift router is directionally worse ($-0.050$ AUROC, raw CI excl.\ $0$, but $n{=}7$ does
not survive Holm). The honest contribution is a cross-domain benchmark with a strong
stacking baseline and a Holm-robust negative result for reliability-aware
routing---not a sealed universal SOTA claim.
\textbf{The decisive update:} in true multimodal detection with independent
modality failure (pre-registered D23, validated on \emph{two} real datasets ---
Real-IAD-D3 (15 categories) and MVTec-3D (7 categories)), reliability routing wins
(beats equal-weight $+0.24$/$+0.21$, stale-select $+0.39$/$+0.32$, validation-only
fusion $+0.25$/$+0.21$; all CIs exclude zero on both), while remaining inert when no
modality fails. This win is, however, \emph{failure-model-specific}: under a realistic
stuck-sensor dropout and under real natural degradation the gate beats stale selection
but not plain fusion (n.s.), so its value needs a detectable drift signal. The precise
claim is therefore \emph{reliability routing helps iff a modality fails independently
\textbf{and detectably}}; for single-input meta-routing plain stacking suffices.
\end{minipage}
\end{titlepage}

\tableofcontents
\listoftables
\listoffigures
\newpage

\section{Executive Summary}

Anomaly detection is fragmented across tabular, cyber, fraud, image-OOD, text,
industrial vision, time-series, and multimodal settings. Each family has strong
specialists, but none is uniformly best. In deployment, the damaging failure is
often not the absence of a useful expert; it is selecting a detector whose
validation performance no longer predicts deployment performance. This is the
negative-transfer problem.

\systemu{} reframes the project around a universal anomaly \emph{meta-router}.
Instead of claiming that one detector is universal, \systemu{} asks whether a
router can reduce average rank, regret to the per-task oracle, and harmful
transfer events across many task families. The project therefore moves from the
old clean-transfer-only framing to a broader \gateu{} contract: success is judged
by cross-family rank, regret, calibration, negative-transfer reduction, and
audited no-test-label routing.

\subsection{Current Evidence Level}

The current thesis evidence should be read in three tiers.

\begin{table}[H]
\centering
\caption{Evidence status of the current ELARA-U thesis.}
\label{tab:evidence-status}
\small
\begin{tabular}{L{0.25\linewidth}L{0.43\linewidth}L{0.22\linewidth}}
\toprule
\textbf{Evidence tier} & \textbf{What it shows} & \textbf{Claim status} \\
\midrule
Clean 123-task benchmark & Rank-normalized stacking beats validation auto-select ($+0.036$ AUROC, CI$>$0) and the best fixed detector ($+0.075$) across 5 families. & Established (verified) \\
Reliability ablation & Reliability/drift features add no significant gain over validation quality; a reliability gate hurts. & Established (negative) \\
Mechanism (simulation) & Reliability gating helps iff a channel fails independently. & Established (simulation) \\
Sealed + independent external & Stacking replicates on a sealed ADBench-extractor holdout (D24) and a fully independent corpus (D27, $+0.111$); reliability wins under multimodal failure (D23). & Done (D23/D24/D27) \\
\bottomrule
\end{tabular}
\end{table}

The immediate research value is that the work now has a defensible thesis:
\emph{meta-routing is most useful when validation metrics become stale under
deployment shift}. This is stronger and cleaner than claiming one fixed detector
or one clean-transfer gate should win everywhere.

\section{Research Questions and Contributions}

\subsection{Research Questions}

\begin{table}[H]
\centering
\caption{Core research questions for the ELARA-U thesis.}
\label{tab:rq}
\small
\begin{tabular}{L{0.08\linewidth}L{0.56\linewidth}L{0.24\linewidth}}
\toprule
\textbf{ID} & \textbf{Question} & \textbf{Current status} \\
\midrule
RQ1 & Can a cross-domain router outperform the best fixed detector family over many anomaly tasks? & Yes (clean 123-task benchmark, CI$>$0) \\
RQ2 & Can the router (stack) outperform validation-AUROC auto-selection? & Yes ($+0.036$ AUROC, CI$>$0) \\
RQ3 & Does reliability/drift modeling, rather than plain stacking, carry the improvement? & \textbf{NO single-input; YES multimodal} (3 single-input regimes vs D23, Table~\ref{tab:d23}) \\
RQ4 & Can the same contract hold on natural, sealed, and external benchmarks (incl.\ multimodal)? & Largely yes: sealed (D24), fully independent (D27), multimodal (D23); natural shift (D22) mixed \\
RQ5 & Can the router produce calibrated probabilities and safe abstentions suitable for operations? & Partially supported; calibration pending \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Contributions}

\begin{longtable}{L{0.22\linewidth}L{0.47\linewidth}L{0.22\linewidth}}
\caption{Contribution matrix.}\label{tab:contrib}\\
\toprule
\textbf{Contribution} & \textbf{Description} & \textbf{Evidence} \\
\midrule
\endfirsthead
\toprule
\textbf{Contribution} & \textbf{Description} & \textbf{Evidence} \\
\midrule
\endhead
Universal meta-routing formulation & Defines anomaly detection as a task-level routing problem over a detector zoo, not as a single-detector competition. & Formal setup and \gateu{} \\
Score archive & Persists validation/test detector scores, labels, and provenance so routing policies can be evaluated without re-running detectors. & $123$ current tasks \\
Rank-normalized stacker & Learns per-task combinations from validation labels and applies monotone-invariant rank features to test scores. & Mean rank $2.53$ (clean benchmark) \\
Evidence contract & Separates average rank, regret, negative transfer, calibration, and no-test-label audit. & Tables and algorithms \\
Theory stack & Proves why universal dominance is impossible, why auto-select is near-oracle under stable transfer, and why shifted regimes can reward reliability routing. & Theorems in Sec.~\ref{sec:theory} \\
Honest boundary & Distinguishes development stress evidence from sealed universal SOTA evidence. & Validity audit \\
\bottomrule
\end{longtable}

\section{Problem Definition}

Let $\mathcal{T}$ be a collection of anomaly-detection tasks. Each task
$\tau\in\mathcal{T}$ has a training split, a validation split, and a deployment or
test split. A detector zoo
\[
\mathcal{D}=\{d_1,\ldots,d_M\}
\]
produces detector scores $s_{\tau,m}(x)$ for every sample $x$. The objective is
not to find a detector that wins every task. The objective is to learn or define a
router $R$ that maps task evidence into a selected, fused, or stacked score
\[
g_\tau(x)=R\left(\{s_{\tau,m}\}_{m=1}^{M}, \Phi_\tau\right)(x),
\]
where $\Phi_\tau$ contains validation-only and unlabeled deployment-shift
features. The router is evaluated using test labels only after routing decisions
are frozen.

\subsection{Metrics}

\begin{table}[H]
\centering
\caption{Primary metrics used by the thesis.}
\label{tab:metrics}
\small
\begin{tabular}{L{0.25\linewidth}L{0.47\linewidth}L{0.20\linewidth}}
\toprule
\textbf{Metric} & \textbf{Definition} & \textbf{Preferred direction} \\
\midrule
Mean rank & Average rank of each method by task AUROC. & Lower \\
Regret & Per-task oracle AUROC minus method AUROC. & Lower \\
Negative-transfer rate & Fraction of tasks where the method underperforms a safe reference such as rank-mean. & Lower \\
Bootstrap CI & Paired task-level confidence interval for rank delta. & Exclude zero \\
ECE proxy & Calibration error treating normalized scores as probabilities. & Lower \\
Worst rank/regret & Worst observed task-level failure. & Lower \\
\bottomrule
\end{tabular}
\end{table}

\section{Related Work}
\label{sec:related}

\paragraph{Benchmarks and the no-free-lunch result.} A consistent finding across
large anomaly-detection studies is that \emph{no single detector dominates}: ADBench
\cite{adbench_th} evaluates 30 algorithms over 57 tabular, vision, and language
feature sets and reports that the best method varies by dataset; PyOD~\cite{pyod_th}
standardises the detector implementations we use. This per-dataset variability is the
empirical premise of our work --- it is exactly what makes a routing/combination
layer plausible --- and our 123-task suite is built from ADBench-style feature
vectors across five families, extended here with time-series (NAB, SMD) and a sealed
representation-shift holdout.

\paragraph{Model and detector selection.} The natural response to no-free-lunch is to
\emph{select} the right detector per dataset. MetaOD~\cite{metaod_th} learns this
mapping from dataset meta-features via collaborative filtering; classical AutoML
selectors and landmarker-based meta-learning follow the same template. The strong
label-aware analogue is validation-AUROC auto-selection, which is our primary
baseline. A central contribution of this chapter is the (perhaps surprising) negative
result that selection --- even a learned MetaOD-style meta-selector --- is the
\emph{wrong primitive}: it does not beat naive auto-select on our suite, and is itself
beaten by stacking (Section~\ref{sec:strength}).

\paragraph{Ensembles, fusion, and stacking.} An alternative to selection is to
\emph{combine} detectors. Score normalisation and averaging, rank aggregation, and
greedy ensemble selection are common in outlier-ensemble work; stacked generalisation
\cite{wolpert_th} fits a meta-model over base-learner outputs. We find that a
rank-normalised logistic stack is the strongest combiner in this family --- it beats
selection with confidence intervals excluding zero, is robust to the stacker family
(random forest, gradient boosting), and \emph{outperforms} the simple average, which
itself loses to selection. The contribution is not a new combiner but a clean,
hardened demonstration of \emph{when} combination beats selection across heterogeneous
anomaly families.

\paragraph{Distribution shift, drift, and selective prediction.} Test-time shift is
classically detected with two-sample statistics such as Kolmogorov--Smirnov, and
selective prediction / learning-to-abstain~\cite{geifman_th} motivates a fallback
action when no expert is trustworthy. Our \emph{reliability} machinery (validation--test
KS drift, calibration, inter-detector disagreement) is built from exactly these
signals. Our honest finding is a boundary result: these signals do \emph{not} add
value over plain stacking in single-input regimes, and help only when modalities can
fail \emph{independently} (Section~\ref{sec:d23}).

\paragraph{Calibration.} Beyond ranking quality, deployable detectors need calibrated
scores. We report fitted Brier and negative log-likelihood under
isotonic-regression calibration~\cite{guo2017calibration}, rather than a raw ECE proxy.

\paragraph{Multimodal and industrial anomaly detection.} Industrial inspection
provides genuinely multimodal data (RGB, photometric stereo, point cloud): PatchCore
\cite{roth2022patchcore} and Real-IAD supply the detectors and co-observed modalities
behind our independent-modality-failure study (D23), the one regime where reliability
routing wins over plain fusion (and then only under a drift-inducing failure). Deep one-class methods~\cite{bergman2020classification} and
OOD baselines~\cite{hendrycks2017baseline} are included in or motivate our detector
zoo and image-OOD family.

\paragraph{Positioning.} In short, prior work establishes (i) that no detector is
universal, and (ii) two competing remedies --- meta-selection and ensembling. This
chapter adjudicates between them on a broad, pre-registered, leakage-audited suite and
reaches a sharp conclusion: \emph{stacking beats selection; reliability routing helps
only under independent modality failure}.

\section{From the Old ELARA System to ELARA-U}

The prior \system{}/\method{} work remains useful, but it should be treated as
source-system evidence rather than the main flagship claim. Its value is threefold:
it provides a detector-and-fusion foundation, it supplies negative evidence about
near-ceiling clean transfer, and it motivates a broader meta-routing benchmark.

\begin{table}[H]
\centering
\caption{How old artifacts should be reused in the new thesis.}
\label{tab:reuse}
\small
\begin{tabular}{L{0.24\linewidth}L{0.42\linewidth}L{0.24\linewidth}}
\toprule
\textbf{Old artifact} & \textbf{Reuse in ELARA-U} & \textbf{Do not use as} \\
\midrule
RGA/RGA+ source system & Detector/fusion foundation and stress mechanism. & Universal proof \\
Clean-transfer failures & Motivation for the new routing objective. & A hidden pass \\
T1--T9 theory stack & Boundary theorems explaining when clean transfer is hard. & A reason to overclaim \\
Real-IAD / Real-IAD D3 & Natural-degradation evidence track and future family input. & Fresh sealed evidence if already opened \\
MVTec 3D-AD / 3D-ADAM / MulSen & Multimodal source families and development evidence. & Final universal holdout without preregistration \\
\bottomrule
\end{tabular}
\end{table}

This distinction is important. A failed clean-transfer gate does not make the
project worthless; it says that the old claim was too narrow. The stronger
scientific move is to make the failure part of the motivation: when clean
validation-to-test transfer is stable, validation auto-selection is already a
strong baseline. A universal paper should therefore focus on the regimes where
selection becomes stale.

\section{Methodology: The Reliability-Aware Meta-Router}

\systemu{} operates in two conceptually distinct modes.

\begin{table}[H]
\centering
\caption{ELARA-U operating modes.}
\label{tab:modes}
\small
\begin{tabular}{L{0.20\linewidth}L{0.42\linewidth}L{0.26\linewidth}}
\toprule
\textbf{Mode} & \textbf{Behavior} & \textbf{Best use} \\
\midrule
Clean baseline mode & Select the detector with highest validation AUROC. & Stable validation-to-test transfer \\
Shift-aware stack mode & Fit a regularized rank-normalized stacker on validation data and apply it to test ranks. & Stale validation under score shift or degradation \\
Fallback mode & Use rank-mean or abstain when channels are degenerate. & Unsafe or untrusted routing \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Detector Zoo and Router Features}

\begin{table}[H]
\centering
\caption{Detector zoo. Current archive uses the tabular/cyber/fraud/image/text subsets; the remaining families are thesis expansion targets.}
\label{tab:zoo}
\small
\begin{tabular}{L{0.22\linewidth}L{0.62\linewidth}}
\toprule
\textbf{Domain} & \textbf{Experts} \\
\midrule
Tabular & ECOD, COPOD, Isolation Forest, LOF, KNN, OCSVM \\
Cyber/fraud & One-class and density detectors over transaction or network features \\
Image-OOD & Feature-space anomaly detectors over CNN representations \\
Text & Feature-space anomaly detectors over sentence/document embeddings \\
Time series & Matrix profile, forecasting residual, reconstruction, and sequence models \\
Industrial vision & PatchCore, reverse-distillation, EfficientAD-style experts \\
Multimodal/3D & RGB, depth, point-cloud, SAR/CW-style multimodal fusions \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[H]
\centering
\caption{Router feature families.}
\label{tab:feat}
\small
\begin{tabular}{L{0.22\linewidth}L{0.50\linewidth}L{0.18\linewidth}}
\toprule
\textbf{Group} & \textbf{Examples} & \textbf{Labels} \\
\midrule
Validation reliability & validation AUROC, spread, confidence, inversion flags & validation only \\
Score geometry & entropy, sharpness, tail mass, saturation, rank dispersion & no labels \\
Disagreement & inter-expert rank and score disagreement & no labels \\
Shift & validation-to-test KS drift, score distribution drift & no test labels \\
Dataset meta-features & sample count, dimension, anomaly rate on validation & validation only \\
Calibration & ECE proxy, future Brier/NLL after fitted calibration & validation only \\
\bottomrule
\end{tabular}
\end{table}

\input{ELARA_U_ALGORITHMS_AND_THEOREMS.tex}

\section{Theory Stack}
\label{sec:theory-intro}

The theory stack is not a proof that \systemu{} is universal SOTA. It is a proof
of when the claim is possible and when it is structurally impossible. The key
message is:
\[
\text{ELARA-U helps when stale-validation regret } \Delta \text{ exceeds router prediction error } 2\eta.
\]
If validation and deployment detector rankings remain stable, auto-selection is
already near-oracle. If deployment shift changes detector ranking and the router
can predict that change from validation-only and unlabeled shift evidence, routing
can reduce regret.

\input{ELARA_U_THEORY_MAIN.tex}

\section{Benchmark Suite and Universal Evidence Contract}

\gateu{} is the evidence contract for the new thesis. It is broader than the old
clean-transfer gate and does not require a single method to be best on every
task. Instead, it asks whether the router reduces rank and regret across a
frozen task suite while avoiding hidden test-label tuning.

\begin{table}[H]
\centering
\caption{Universal Evidence Contract.}
\label{tab:contract}
\small
\begin{tabular}{L{0.34\linewidth}L{0.44\linewidth}L{0.14\linewidth}}
\toprule
\textbf{Endpoint} & \textbf{Pass condition} & \textbf{Current status} \\
\midrule
Average rank & Router mean rank below auto-select and best fixed detector with paired CI excluding zero. & Supported in stress archive \\
Mean regret & Router regret below auto-select and best fixed detector. & Supported in stress archive \\
Negative-transfer rate & Router negative-transfer rate below safe reference or auto-select. & Partially supported \\
Family robustness & Gains on multiple families, not only one category. & Partial \\
Calibration & ECE/Brier/NLL improve after a fitted calibration layer. & Done (Table~\ref{tab:calib_th}) \\
No test-label tuning & Routing decisions use validation labels and unlabeled test scores only. & Audited in code path \\
External seal & At least one frozen external/leaderboard-style benchmark. & Done (D24, Table~\ref{tab:sealed_th}) \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[H]
\centering
\caption{Current and planned benchmark coverage.}
\label{tab:manifest}
\small
\begin{tabular}{L{0.20\linewidth}L{0.48\linewidth}L{0.22\linewidth}}
\toprule
\textbf{Family} & \textbf{Datasets / tasks} & \textbf{Status} \\
\midrule
Tabular & ADBench-style datasets and online shoppers & Current archive \\
Cyber & UNSW-NB15 full and attack-family tasks & Current archive \\
Fraud & Credit-card $+$ BAF 6-variant panel ($n{=}7$) & Current archive; stacking n.s.\ on near-chance BAF (boundary, D26) \\
Image-OOD & CIFAR10, FashionMNIST, SVHN, MNIST-C, MVTec-AD feature tasks & Current archive \\
Text & 20News, AGNews, Amazon, IMDB, Yelp feature tasks & Current archive \\
Industrial vision v2 & MVTec AD 2 and related real industrial benchmarks & Planned \\
Time series & TSB-AD and native temporal anomaly suites & Planned \\
Multimodal/3D & Real-IAD D3, MVTec 3D-AD, 3D-ADAM, MulSen & Planned integration \\
\bottomrule
\end{tabular}
\end{table}

\section{Experimental Evaluation}

The canonical archive contains $123$ tasks across five families. The verified
headline (Table~\ref{tab:main}, \texttt{honest\_benchmark.json}) is on the
\emph{clean} benchmark: an earlier evaluator injected a constant score offset as a
``stress'' test, but a constant offset is AUROC-invariant (it preserves every
detector's ranking), so it did not materially change the result---clean and
``degraded'' numbers coincide. We therefore report the clean benchmark, where the
comparison is honest and unconfounded: \systemu{} Stack versus validation-AUROC
auto-selection versus fixed detectors.

\subsection{Headline Result}\label{sec:headline}

On the clean 123-task benchmark, the rank-normalized logistic stack is the strongest
non-oracle strategy and even exceeds the best-single-detector oracle (fusion beats
selection):
\begin{align*}
\text{Stack: } &\text{rank}=2.53, & \text{AUROC}&=0.784, & \text{ECE}&=0.271,\\
\text{Auto-select: } &\text{rank}=3.38, & \text{AUROC}&=0.748, & \text{ECE}&=0.393.
\end{align*}
The paired bootstrap over 123 tasks gives Stack $-$ auto-select $=+0.036$ AUROC
(95\% CI $[0.023,0.050]$), Stack $-$ best fixed $=+0.075$ ($[0.052,0.101]$), and
auto-select $-$ best fixed $=+0.040$ ($[0.023,0.061]$); all exclude zero. This
answers RQ1 and RQ2 affirmatively.

\subsection{Interpretation: RQ3 resolved (negative)}

The decisive question is RQ3: does the gain come from \emph{reliability/drift}
modeling, or from plain stacking? The verified ablations (Table~\ref{tab:ablation})
answer it: a reliability gate on the stack significantly \emph{hurts} ($-0.037$ AUROC,
$[-0.050,-0.026]$); a learned router with the full reliability battery is no better
than the same router without it (i.i.d.\ $+0.001$, uniform shift $+0.002$, heterogeneous
missingness $+0.015$; all CIs cross zero) and worse than plain auto-select on i.i.d.\
data. Across two router designs and three deployment regimes, reliability features
carry no significant signal beyond validation quality and detector identity. Crucially this holds on a \emph{sealed, natural}
temporal-shift benchmark (D22: UNSW capture periods and a credit-card time split,
late period scored once): there the drift-aware router is significantly
\emph{worse} than plain stacking ($-0.050$ AUROC, 95\% CI $[-0.103,-0.013]$) and
only $+0.008$ over auto-select (not significant), so the negative result is not an
artifact of synthetic corruption.

The honest conclusion is therefore precise: \emph{the universal-routing gain is real
but comes from rank-normalized stacking, not from reliability-aware routing.} The next
subsection explains \emph{why} reliability fails here, and Section~\ref{sec:multimodal}
pre-registers the one regime where it should structurally succeed.

\subsection{Mechanism: reliability needs an independent clean channel}\label{sec:mechanism}

Why does a structurally reasonable idea fail? A controlled simulation isolates the
single deciding variable: whether a channel can fail independently while the others
stay clean. In a \textbf{SHARED} regime all channels weaken together (the
single-modality analogue---one input feeds every detector); in an \textbf{INDEPENDENT}
regime one channel loses its signal while the rest stay clean (genuine multimodal).
The reliability gate, auto-select, and the no-test-time ablation are identical across
regimes.

\begin{table}[H]\centering
\caption{Mechanism simulation (300 trials/regime, paired bootstrap). Reliability gating
helps iff a channel fails independently.}
\label{tab:mechanism}\footnotesize
\begin{tabular}{lccc}
\toprule
\textbf{Regime} & \relg{} $-$ mean & \relg{} $-$ auto-select & \relg{} $-$ ablation \\
\midrule
SHARED (all weaken together) & $-0.016$ (ns) & $+0.081$ & $-0.019$ (ns) \\
INDEPENDENT (one fails, rest clean) & $+0.026$ \checkmark & $+0.144$ \checkmark & $+0.024$ \checkmark \\
\bottomrule
\end{tabular}
\end{table}

Under SHARED degradation reliability gating cannot help---there is no clean channel to
switch to---matching the single-modality benchmark. Under INDEPENDENT failure it beats
equal-weight fusion, stale selection, and its own no-test-time ablation, all with
CI$>0$; the unlabeled test-time reliability signal, not validation quality, carries the
gain. Single-modality detectors share one input, so deployment degradation is shared and
the enabling condition is violated. This is exactly the antecedent of the
independent-failure theorem (Sec.~\ref{sec:theory}): the negative result is
theory-consistent, not a refutation of reliability gating in general.

\subsection{Honest boundary}

The result is not a sealed universal SOTA claim, for three reasons. First, the
significant stacking advantage is established on families with \emph{individually
informative} base detectors (tabular, image-OOD, text, cyber); no industrial-vision
or significant time-series claim is made, and on the near-chance BAF fraud panel
(D26) stacking is explicitly not significant --- the win is bounded to where the zoo
carries signal. Second, the multimodal reliability mechanism is now backed by real
co-observed data (D23, Section~\ref{sec:multimodal}), not only the controlled
simulation, and the stacking result now replicates both on a sealed representation-shift
holdout (D24, Table~\ref{tab:sealed_th}) \emph{and} on a fully independent external
corpus from distinct sources (D27, Table~\ref{tab:indep_th}, $+0.111$), so the
``opened-data'' concern is directly addressed. Third, the fraud family is now $n{=}7$
(credit-card $+$ BAF) but the BAF result is a boundary, not a win, and sealed
industrial 3D holdouts beyond Real-IAD-D3 remain pending.

\input{ELARA_U_RESULT_SCAFFOLD.tex}

\section{Validity and Leakage Audit}

The paper must be strict about what information the router may use. The current
router is not fully unsupervised: it uses validation labels to fit per-task
rank-normalized logistic stacking. That is acceptable if the benchmark contract
states it clearly. It is not acceptable to describe the router as label-free.

\begin{table}[H]
\centering
\caption{Leakage and validity audit.}
\label{tab:leakage}
\small
\begin{tabular}{L{0.26\linewidth}L{0.42\linewidth}L{0.22\linewidth}}
\toprule
\textbf{Risk} & \textbf{Current control} & \textbf{Remaining work} \\
\midrule
Test-label routing & Routing uses validation labels and unlabeled test scores; test labels score only after routing. & \textbf{Done}: executable no-test-label suite (\texttt{tests/elara\_u/test\_no\_test\_label\_leakage.py}; $6$ guards over stacking, router, and meta-selection paths) \\
Opened data & Current archive is development evidence. & \textbf{Done}: sealed external holdout frozen \emph{before} scoring (D24, Table~\ref{tab:sealed_th}) \\
Synthetic degradation & Stress benchmark is useful but not natural evidence. & \textbf{Partly done}: natural temporal shift (D22) and time-series (NAB, SMD) added; native sensor/domain shift remains \\
Overfitting to families & Leave-family-out is used, but only five families exist. & Add more families and nested meta-validation \\
Calibration claims & ECE proxy is reported, not true probability calibration. & \textbf{Done}: isotonic calibrator fitted; Brier/NLL reported (Table~\ref{tab:calib_th}) \\
Ablation weakness & Reliability/drift ablations are currently not decisive. & \textbf{Resolved}: claim narrowed to rank stacking; reliability validated only under independent multimodal failure (D23) \\
\bottomrule
\end{tabular}
\end{table}

\section{Pre-registered Multimodal Test: Run and Passed on Two Real Datasets}\label{sec:multimodal}

The mechanism (Sec.~\ref{sec:mechanism}) and the independent-failure theorem
(Sec.~\ref{sec:theory}) make a falsifiable prediction for genuine multimodal anomaly
detection (RGB $+$ 3D), where one modality can fail independently. We pre-register the
test with a \emph{frozen decision rule and no post-hoc retuning}. Under independent
per-modality deployment degradation (one modality degrades at test; the other and the
validation split stay clean), reliability-gated fusion must beat:
\begin{itemize}
\item[\textbf{H1}] equal-weight fusion, paired-bootstrap CI$>0$;
\item[\textbf{H2}] stale validation-AUROC selection, CI$>0$;
\item[\textbf{H3}] its own no-test-time-reliability ablation, CI$>0$ (the test-time
reliability signal, not validation quality, must carry the gain).
\end{itemize}
The statistical unit is (category $\times$ degradation-seed); MVTec-3D's ten categories
alone are too few. \textbf{Decision rule:} all of H1--H3 significant $\Rightarrow$ a real,
scoped novel-method claim (reliability gating recovers anomaly detection under
per-modality deployment failure). Any of H1--H3 fails $\Rightarrow$ the reliability
premise is closed even where it should structurally hold, reported as the final
boundary, with no novel claim and no retuning. A CPU positive control with the correct
design passes decisively (H1 $+0.094$, H2 $+0.264$, H3 $+0.097$, all CI$>0$).

\paragraph{Result: the test was run and \emph{passed} on two real datasets
(Table~\ref{tab:d23}).} Using per-modality PatchCore experts (RGB, PS, and XYZ
point-cloud geometry), we built one-class memory banks on the normal split and scored
a held-out val/test split per category, then failed the deployment-best modality at
test (validation stays clean). On \textbf{Real-IAD-D3} (15 categories) and
\textbf{MVTec-3D} (7 categories), the reliability gate beats all three baselines on
\emph{both} datasets: H1 vs equal-weight $+0.241$/$+0.207$, H2 vs stale auto-select
$+0.386$/$+0.317$, H3 vs the no-test-time-reliability ablation $+0.248$/$+0.211$
(Real-IAD-D3/MVTec-3D), with every $95\%$ paired-bootstrap CI excluding zero; the gate
is inert in the clean regime (equals validation-only fusion). \textbf{All of H1--H3
pass on two independent real datasets}, so the pre-registered decision rule is met:
this establishes the scoped novel claim---reliability routing recovers anomaly
detection under independent per-modality failure. In the Holm-corrected consolidated
ablation (Table~\ref{tab:ablation}) this is the \emph{only} regime where reliability
significantly helps ($p_{\mathrm{Holm}}<10^{-8}$ on Real-IAD-D3), against null/negative
across all single-input regimes. The harness
(\path{src/scripts/elara_u/multimodal_reliability_test.py}) and frozen protocol
(\path{research_lock/MULTIMODAL_RELIABILITY_PROTOCOL_v1.md}) are released.
\emph{Scope:} the modality failure is injected at deployment (the pre-registered
design); a naturally-occurring-failure version is the next extension.

\section{Why This Can Become SOTA-Level}

The SOTA-level potential is not the claim that \systemu{} is already best
everywhere. The positive contribution is a strong, under-reported universal stacking
baseline; the scientific contribution is the honest boundary---\emph{where}
reliability-aware routing helps and where it cannot, with a mechanism and a
pre-registered test \emph{now run and passed} (Sec.~\ref{sec:multimodal}, two real
datasets) for the one regime that satisfies the condition. This targets a real gap: detector papers are domain-specific, while
deployment needs an honestly-characterized decision layer.

\begin{table}[H]
\centering
\caption{Path from current thesis evidence to SOTA-level evidence.}
\label{tab:sota-path}
\small
\begin{tabular}{L{0.22\linewidth}L{0.46\linewidth}L{0.22\linewidth}}
\toprule
\textbf{Requirement} & \textbf{What must be added} & \textbf{Why it matters} \\
\midrule
Natural shift & Time-based fraud, native cyber drift, real sensor degradation, industrial test splits. & Replaces stress-only evidence \\
More families & Time-series, industrial vision v2, multimodal/3D, OOD leaderboards. & Makes ``universal'' credible \\
Sealed benchmark & Freeze protocol before scoring at least one external holdout. & Prevents opened-test selection \\
Multimodal reliability test & \textbf{Done}: the pre-registered RGB$+$3D test (Sec.~\ref{sec:multimodal}) passed on Real-IAD-D3 + MVTec-3D (H1--H3 all CI$>$0). & Scoped novelty established (reliability wins under independent modality failure) \\
Calibration layer & Report Brier, NLL, ECE, abstention curves. & Supports operational reliability \\
Baselines & Compare with AutoML/ensemble/stacking, detector selection, rank aggregation, and recent AD methods. & Satisfies senior reviewers \\
Release package & Code, score archive, configs, checksums, and reproducible runbook. & Makes the paper verifiable \\
\bottomrule
\end{tabular}
\end{table}

\section{Expanded Experimental Plan}

The next experimental stage should turn the current development stress result
into a sealed universal benchmark.

\begin{longtable}{L{0.18\linewidth}L{0.39\linewidth}L{0.31\linewidth}}
\caption{Recommended next experiments for the thesis.}\label{tab:next-exp}\\
\toprule
\textbf{Experiment} & \textbf{Design} & \textbf{Pass/fail rule} \\
\midrule
\endfirsthead
\toprule
\textbf{Experiment} & \textbf{Design} & \textbf{Pass/fail rule} \\
\midrule
\endhead
Clean archive re-run & Evaluate auto-select, rank-mean, fixed detectors, and stacker without degradation. & Expect tie or modest gains; do not require dominance \\
Natural temporal split (D22, \textbf{done}) & UNSW capture periods + credit-card time split; sealed late period scored once. & \textbf{FAILED}: drift-stack $-0.050$ vs plain stacking (CI excl.\ 0); $+0.008$ vs auto-select (n.s.) \\
Industrial natural shift & Real-IAD D3 / MVTec variants with natural acquisition degradation. & Beat auto-select and rank-mean without synthetic corruption \\
Multimodal RGB$+$3D (\textbf{done}) & Independent per-modality degradation; reliability-gated fusion vs mean, auto-select, ablation (Sec.~\ref{sec:multimodal}). & \textbf{Passed} on Real-IAD-D3 (15 cat) + MVTec-3D (7 cat): H1--H3 all CI$>$0 $\Rightarrow$ scoped novelty established \\
Calibration experiment & Fit calibration on validation only; test on held-out tasks. & Improve ECE, Brier, and NLL \\
Abstention curve & Allow router to abstain on low-confidence tasks. & Lower regret among fired tasks and bounded abstention rate \\
External seal & Freeze code and score a new benchmark or leaderboard. & No post-hoc tuning; CI must support the main claim \\
\bottomrule
\end{longtable}

\section{Production and Release Boundary}

Scientific readiness and production readiness are separate. A production API can
be secure, reproducible, monitored, and honest while the scientific SOTA claim
remains pending. The release should therefore present two statuses:

\begin{table}[H]
\centering
\caption{Release boundary.}
\label{tab:release-boundary}
\small
\begin{tabular}{L{0.24\linewidth}L{0.44\linewidth}L{0.22\linewidth}}
\toprule
\textbf{Status} & \textbf{Meaning} & \textbf{Current position} \\
\midrule
Operational readiness & API/runtime is secure, authenticated, monitored, reproducible, and honest about model availability. & Separate production track \\
Scientific readiness & Frozen external evidence supports the main research claim. & Development thesis evidence, not final SOTA \\
Thesis readiness & The problem, method, theory, and pilot evidence are coherent enough for a PhD chapter. & Strong if boundaries remain explicit \\
\bottomrule
\end{tabular}
\end{table}

\section{Reproducibility}

\begin{table}[H]
\centering
\caption{Current reproducibility map.}
\label{tab:repro-map}
\small
\begin{tabular}{L{0.33\linewidth}L{0.52\linewidth}}
\toprule
\textbf{Artifact} & \textbf{Path} \\
\midrule
Score archive builder & \path{src/scripts/elara_u/build_score_archive.py} \\
Universal contract evaluator & \path{src/scripts/elara_u/evaluate_universal_contract.py} \\
Stacker ablation runner & \path{src/scripts/elara_u/run_stacker_ablations.py} \\
Heterogeneous degradation ablation & \path{src/scripts/elara_u/heterogeneous_degradation_ablation.py} \\
Router implementation & \path{src/uais/elara_u/router.py} \\
Metrics implementation & \path{src/uais/elara_u/contract.py} \\
Canonical results & \path{experiments/elara_u/results.json} \\
Generated result tables & \path{docs/research/ELARA_U_RESULT_TABLES.tex} \\
Generated figures & \path{docs/research/figures/} \\
Protocol & \path{research_lock/GATE_U_CROSS_DOMAIN_PROTOCOL_v1.yaml} \\
\bottomrule
\end{tabular}
\end{table}

\section{Strengthening the Stacking Result}
\label{sec:strength}
Five reviewer-hardening checks confirm the main (single-input) result is robust:
a comprehensive baseline comparison, stacker-family invariance, detector-zoo
strength, calibration, and external replication. We first establish that the win is
not an artefact of weak baselines.

\paragraph{Comprehensive baseline comparison (D28).}
The strongest objection to a stacking result is that the baselines are weak. We
therefore compare the headline logistic stack against \emph{every} standard
combiner, selector, and ensemble, plus two test-label oracle ceilings, on the
123-task archive (Table~\ref{tab:baselines_th}). The stack beats all nine non-oracle
baselines with CIs excluding zero --- selection (best fixed, auto-select, a learned
MetaOD-style selector), averaging (raw, rank, confidence-weighted, calibrated), an
AutoML-style greedy ensemble (Caruana), and a per-family specialist stacker.
Decisively, it also beats the \emph{oracle best single detector} ($+0.025$, CI
$[+0.012,+0.039]$) and the \emph{oracle best equal-weight pair} ($+0.032$, CI
$[+0.018,+0.048]$), both of which use test labels to pick. A weak-baseline win cannot
exceed the oracle selection ceiling; a genuine $6$-way per-sample combination can.

\begin{table}[H]\centering
\caption{Comprehensive baseline comparison (123 tasks; paired bootstrap of headline
logistic stack minus each method). Beats every non-oracle baseline and both
test-label oracle ceilings; CIs exclude zero except where n.s.}
\label{tab:baselines_th}\small
\begin{tabular}{lcc}
\toprule
\textbf{Method} & mean AUROC & stack $-$ method (95\% CI) \\
\midrule
\multicolumn{3}{l}{\emph{Selection}}\\
Best fixed detector & 0.709 & $+0.073\,[+0.050,+0.099]$ \\
Validation auto-select & 0.748 & $+0.033\,[+0.021,+0.047]$ \\
MetaOD-style learned select & 0.736 & $+0.045\,[+0.031,+0.061]$ \\
\multicolumn{3}{l}{\emph{Averaging / ensemble}}\\
Rank-mean & 0.707 & $+0.074\,[+0.057,+0.093]$ \\
Confidence-weighted mean & 0.708 & $+0.073\,[+0.055,+0.093]$ \\
Simple average (raw) & 0.709 & $+0.072\,[+0.054,+0.092]$ \\
Average (calibrated) & 0.734 & $+0.048\,[+0.035,+0.062]$ \\
AutoML greedy ensemble & 0.749 & $+0.032\,[+0.020,+0.046]$ \\
\multicolumn{3}{l}{\emph{Stackers}}\\
Per-family specialist & 0.732 & $+0.049\,[+0.034,+0.066]$ \\
Stack (random forest) & 0.771 & $+0.010$ (n.s.) \\
Stack (grad.\ boosting) & 0.763 & $+0.019\,[+0.005,+0.033]$ \\
Stack (hist-GBM) & 0.769 & $+0.012$ (n.s.) \\
Stack (logistic, rank-norm) & 0.783 & $-0.003$ (n.s.) \\
\textbf{Stack (logistic, headline)} & \textbf{0.781} & --- \\
\multicolumn{3}{l}{\emph{Oracle upper bounds (use test labels)}}\\
Oracle best single detector & 0.756 & $+0.025\,[+0.012,+0.039]$ \\
Oracle best pair fusion & 0.749 & $+0.032\,[+0.018,+0.048]$ \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[H]\centering
\caption{Stacking beats selection across stacker families (123 tasks; paired
bootstrap vs auto-select). RF/GBM match logistic (n.s.); a simple average loses,
so the gain is learned combination, not averaging.}
\label{tab:stackers_th}\small
\begin{tabular}{lccc}
\toprule
\textbf{Method} & mean AUROC & $\Delta$ vs auto-select & 95\% CI \\
\midrule
Auto-select & 0.748 & --- & --- \\
Meta-select (MetaOD-style, learned) & 0.737 & $-0.011$ & $[-0.017,-0.006]$ \\
Simple average & 0.709 & $-0.039$ & $[-0.048,-0.031]$ \\
Stack (logistic) & 0.781 & $+0.033$ & $[+0.021,+0.047]$ \\
Stack (random forest) & 0.770 & $+0.022$ & $[+0.003,+0.041]$ \\
Stack (grad.\ boosting) & 0.774 & $+0.026$ & $[+0.009,+0.044]$ \\
Stack ($+$ DeepSVDD in zoo) & 0.784 & $+0.038$ & $[+0.025,+0.051]$ \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[H]\centering
\caption{Calibration (lower better): a validation-only isotonic layer makes the
stack well-calibrated, far better than auto-select.}
\label{tab:calib_th}\small
\begin{tabular}{lccc}
\toprule
\textbf{Method} & ECE & Brier & NLL \\
\midrule
Auto-select & 0.393 & 0.249 & 0.743 \\
Stack (raw) & 0.280 & 0.173 & 0.514 \\
Stack $+$ isotonic & \textbf{0.024} & \textbf{0.069} & \textbf{0.276} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Stacking beats \emph{learned} selection too (D25).}
The strongest form of the ``you only beat argmax auto-select'' objection is a
\emph{learned} meta-selector such as MetaOD~\cite{metaod_th}, which predicts the
best model from dataset meta-features. We implement an in-repo MetaOD-style selector
(a random forest over $28$ validation-only meta-features --- including the
validation-AUROC landmarkers --- trained leave-one-task-out to predict each
detector's test AUROC, then selecting the predicted-best detector; honest scope:
this is our reimplementation in MetaOD's spirit, not the published artefact).
Strikingly, learned selection does \emph{not} beat naive auto-select: it scores
$0.737$, \emph{below} auto-select ($-0.011$, CI $[-0.017,-0.006]$, n.\,tasks $=123$).
Per-task stacking beats this learned selector by $+0.044$ (CI $[+0.030,+0.060]$).
The lesson is sharp: the gain is not from picking a \emph{better} detector --- even a
meta-learned pick underperforms --- but from \emph{combining} the detector scores.
Selection, however clever, is the wrong primitive here.

Adding a deep detector (DeepSVDD) does not trivialise routing: DeepSVDD underperforms
the classical detectors here (mean AUROC $0.65$, consistent with ADBench), and the
stacking advantage over auto-select persists ($+0.038$, CI excludes zero). The boundary is also mapped: on two time-series families
(NAB univariate $+0.008$; SMD multivariate $+0.011$; both n.s.) stacking is only
marginally ahead of auto-select.

\paragraph{Fraud-family power and a near-chance boundary (D26).}
The fraud family was the weakest point of the leave-family-out analysis ($n{=}1$:
credit-card). We expand it with the Bank Account Fraud suite~\cite{baf_th}
(Feedzai, NeurIPS 2022): six bias-controlled variants (Base, I--V) of a large
tabular fraud problem, a second fraud source, lifting the family to $n{=}7$. The
result is an honest \emph{negative/boundary} finding. On BAF the entire detector zoo
is near chance (mean AUROC $\approx 0.6$; BAF is engineered to be hard, with label
noise and temporal drift), and there stacking is \emph{not} significantly better
than auto-select ($+0.011$, CI $[-0.066,+0.064]$) and sits \emph{below} the best
fixed detector ($-0.033$, CI $[-0.098,+0.022]$); on Variant~I the per-task logistic
stack even collapses below chance ($0.375$). This refines the central claim: the
significant stacking advantage holds where base detectors are \emph{individually
informative}, and not on near-chance problems --- combination amplifies signal that
exists but cannot conjure it where the zoo is uniformly weak. Together with the
time-series boundary, this confines the significant win to families with informative
base detectors, rather than to ``feature-vector'' families per se (BAF is itself
tabular).

\paragraph{Sealed external replication (D24).}
The strongest guard against an over-fit development split is a \emph{sealed}
holdout: the method, baselines, detector zoo, and pass condition were frozen
(\texttt{research\_lock/SEALED\_EXTERNAL\_PROTOCOL\_v1}) \emph{before} any holdout
data was loaded, then scored exactly once with no post-hoc tuning. The holdout is
$74$ ADBench tasks under a \emph{different feature extractor} than development
(CV\_by\_ViT and NLP\_by\_RoBERTa vs.\ the ResNet18/BERT features used to build the
123-task archive), so these representations were never inspected. On this one-shot
test (Table~\ref{tab:sealed_th}) the stack still beats auto-select ($+0.016$, CI
$[+0.004,+0.032]$) and the best fixed detector ($+0.043$, CI $[+0.022,+0.073]$);
both CIs exclude zero. The effect therefore \emph{replicates under representation
shift}, lifting the stacking result from development evidence to sealed evidence
(Gate~U point~7). Honest scope: this is a sealed held-out under shifted
representation on the same ADBench suite, not a wholly independent data source ---
that gap is closed next (D27).

\begin{table}[H]\centering
\caption{Sealed external holdout (D24, one-shot): method frozen before scoring the
same $74$ ADBench datasets re-featurized with an untouched ViT/RoBERTa extractor
(representation-shift holdout). Stacking beats both auto-select and the best fixed detector; both CIs exclude zero.}
\label{tab:sealed_th}\small
\begin{tabular}{lcc}
\toprule
\textbf{Method} & mean AUROC & $\Delta$ (95\% CI) \\
\midrule
Best fixed detector (KNN) & 0.768 & --- \\
Rank-mean & 0.762 & --- \\
Simple average & 0.764 & --- \\
Auto-select & 0.795 & --- \\
\textbf{Stack (logistic)} & \textbf{0.811} & vs auto $+0.016\,[+0.004,+0.032]$ \\
 & & vs best $+0.043\,[+0.022,+0.073]$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Fully independent external corpus (D27).}
The sealed holdout (D24) reuses the ADBench \emph{suite} under new extractors; the
hardest test is a different data \emph{source} entirely. We froze the method and the
exact $20$-task list (\texttt{research\_lock/INDEPENDENT\_EXTERNAL\_PROTOCOL\_v1})
over sources \emph{verifiably absent} from the 135-task archive --- scikit-learn
\texttt{digits} (10 one-vs-rest tasks), \texttt{wine} (3), \texttt{wdbc}
(malignant $=$ anomaly), and HAR smartphones ($561$ features, 6 activities) --- and
scored once with the construction ``each class $=$ anomaly, downsampled to $10\%$''.
The result is the strongest replication in this thesis: stacking reaches mean AUROC
$0.799$ versus auto-select $0.689$ ($+0.111$, CI $[+0.052,+0.176]$) and the best
fixed detector $0.590$ ($+0.209$, CI $[+0.120,+0.303]$); both CIs exclude zero
(Table~\ref{tab:indep_th}). The advantage is \emph{larger} on these independent
sources than on the development split, because the one-vs-rest tasks have informative
detectors and single-detector selection is weak --- exactly the regime where learned
combination should and does win. This lifts the stacking claim from sealed evidence
to \emph{fully independent external} evidence.

\begin{table}[H]\centering
\caption{Fully independent external corpus (D27, one-shot): method and 20-task list
frozen before scoring sources absent from the development archive (sklearn
digits/wine/wdbc + HAR). Stacking beats auto-select and the best fixed detector; both
CIs exclude zero.}
\label{tab:indep_th}\small
\begin{tabular}{lcc}
\toprule
\textbf{Method} & mean AUROC & $\Delta$ (95\% CI) \\
\midrule
Best fixed detector (COPOD) & 0.590 & --- \\
Rank-mean & 0.596 & --- \\
Simple average & 0.598 & --- \\
Auto-select & 0.689 & --- \\
\textbf{Stack (logistic)} & \textbf{0.799} & vs auto $+0.111\,[+0.052,+0.176]$ \\
 & & vs best $+0.209\,[+0.120,+0.303]$ \\
\bottomrule
\end{tabular}
\end{table}

\section{Multimodal Independent-Modality Failure (D23)}
\label{sec:d23}
Every regime where reliability routing was inert used \emph{single-input}
families: all detectors see the same data, so no modality can fail independently.
We therefore pre-registered (\texttt{research\_lock/MULTIMODAL\_RELIABILITY\_PROTOCOL\_v1})
a test on true multimodal data (Real-IAD-D3: RGB, photometric-stereo, point-cloud
XYZ experts), failing the deployment-best modality at test while the others stay
clean; reliability routing detects the failed modality (KS validation-vs-test
drift, no test labels) and routes away from it. All three pre-registered
hypotheses pass (Table~\ref{tab:d23}); in the clean regime the gate is inert. The
precise claim: \emph{reliability routing helps iff modalities can fail
independently}.

\begin{table}[t]\centering
\caption{D23 multimodal reliability on \emph{two} real datasets (Real-IAD-D3, 15
categories; MVTec-3D, 7 categories). Mean AUROC in the modality-failure regime; the
gate is inert in the clean regime (equals validation-only fusion) on both. Hypotheses
are paired bootstrap over categories.}
\label{tab:d23}\footnotesize
\begin{tabular}{lcc}
\toprule
\textbf{Method (failure regime)} & Real-IAD-D3 & MVTec-3D \\
\midrule
Equal-weight (CW) & 0.526 & 0.586 \\
Stale auto-select & 0.381 & 0.475 \\
Validation-only fusion & 0.519 & 0.581 \\
\textbf{Reliability gate} & \textbf{0.767} & \textbf{0.792} \\
\midrule
\multicolumn{3}{p{0.92\linewidth}}{\emph{Real-IAD-D3:} H1 vs equal-weight $+0.241$ $[0.152,0.348]$; H2 vs stale-select $+0.386$ $[0.281,0.483]$; H3 vs validation-only $+0.248$ $[0.168,0.323]$. \emph{MVTec-3D:} H1 $+0.207$ $[0.172,0.242]$; H2 $+0.317$ $[0.240,0.405]$; H3 $+0.211$ $[0.180,0.244]$. All 95\% CIs exclude zero.}\\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Failure-model dependence and real degradation (honest reality check).}
The D23 wins use the canonical \emph{uniform-noise} modality failure (the failed
sensor emits garbage, which \emph{drifts} detectably). Two checks with more realistic
faults locate the gate's true regime. \textbf{(i) Real sensor dropout} (MulSen-AD,
stuck/constant reading): the gate still beats stale single-modality selection
($+0.28$) but \emph{not} plain fusion (vs equal-weight $-0.03$, n.s.; vs
validation-only fusion $-0.05$). \textbf{(ii) Real natural degradation} (D29:
Real-IAD-D3 per-modality deep-PatchCore scores from the raw $259$\,GB, $16$ categories,
no injection): the gate beats stale selection ($+0.13$) but not plain fusion
($+0.02$/$+0.03$, n.s.). The honest refinement: \emph{fusion robustly beats selection,
but the reliability gate's extra value over fusion is specific to drift-inducing
faults}; under a stuck sensor or mild natural degradation there is no test-time drift to
exploit and plain fusion is as good --- exactly the inertness proposition.

\paragraph{A new method and a conjecture (per-sample routing).} The inertness result is
specific to \emph{per-channel} reweighting (constant across samples). This motivates a
genuinely new combiner: \emph{per-sample reliability-gated stacking} (PS-RGS), whose
weights $w_m(x)$ vary with the input via no-label local-reliability features and thus
lie \emph{outside} the linear class the inertness proposition bounds. We conjecture
(T10) that per-sample gating strictly exceeds the best linear stacker \emph{iff}
per-sample reliability is label-relevant and heterogeneous across the input space. On
the $123$-task suite PS-RGS gives a small directional improvement over plain stacking
($+0.0068$, CI $[-0.0002,+0.015]$; $83/123$ wins) that does \emph{not} reach
significance, and its per-task gain correlates positively but not significantly with
detector heterogeneity (Spearman $r\!\approx\!0.14$, $p\!\approx\!0.12$) --- the right
direction for T10, but underpowered on a homogeneous classical zoo. PS-RGS is therefore
a theoretically-grounded direction with honest \emph{preliminary} evidence, not a
confirmed win.

We then \emph{ran} the natural next experiment --- PS-RGS on a heterogeneous
deep$+$classical zoo (adding DeepSVDD, an autoencoder, and a VAE) --- and pre-registered
a one-shot held-out confirmation (protocol D32) to guard against a development
false-positive. The result is an instructive \textbf{honest negative}. On tabular
\emph{development} data the deep zoo made PS-RGS significantly beat plain stacking
($+0.016$, CI excl.\ $0$); but on the \emph{pre-registered held-out} families the effect
\emph{did not replicate}: ADBench-CV $+0.002$ (n.s.) and ADBench-NLP $-0.020$
(significantly \emph{worse}). The tabular significance was therefore a development-set
artefact, and the pre-registered one-shot caught it. PS-RGS is thus \textbf{not} a
confirmed improvement; we report it as a theoretically-motivated method refuted by
held-out confirmation --- a textbook illustration of why pre-registration is the
backbone of this thesis. The only robust effect from the deep zoo is minor: it modestly
improves \emph{plain} stacking on feature-vector families (tabular $+0.010$, CV $+0.004$;
both CIs exclude $0$; n.s.\ on NLP). No SOTA or breakthrough claim follows: the absolute
levels ($0.87$/$0.78$/$0.65$ across tabular/CV/NLP) are not leaderboard-beating, and the
new method does not even reliably beat plain stacking out-of-family.

\section{Conclusion}

\systemu{} is now best framed as a universal anomaly meta-routing thesis rather
than a clean-transfer superiority paper. The current result is promising:
rank-normalized stacking substantially improves average rank and regret under a
development stress benchmark where validation metrics become stale. The honest
scientific boundary is equally clear: the current evidence is not enough for a
sealed universal SOTA claim, and the reliability/drift novelty must be
strengthened beyond plain stacking. The next phase should therefore prioritize
natural shift, sealed holdouts, stronger ablations, calibrated abstention, and a
clean reproducibility package.

\appendix
\input{ELARA_U_THEORY_APPENDIX.tex}

\begin{thebibliography}{9}
\bibitem{breunig2000lof}
M. M. Breunig, H.-P. Kriegel, R. T. Ng, and J. Sander, ``LOF: Identifying density-based local outliers,'' in \emph{SIGMOD}, 2000.

\bibitem{liu2008iforest}
F. T. Liu, K. M. Ting, and Z.-H. Zhou, ``Isolation forest,'' in \emph{ICDM}, 2008.

\bibitem{bergman2020classification}
L. Bergman and Y. Hoshen, ``Classification-based anomaly detection for general data,'' in \emph{ICLR}, 2020.

\bibitem{roth2022patchcore}
K. Roth et al., ``Towards total recall in industrial anomaly detection,'' in \emph{CVPR}, 2022.

\bibitem{hendrycks2017baseline}
D. Hendrycks and K. Gimpel, ``A baseline for detecting misclassified and out-of-distribution examples in neural networks,'' in \emph{ICLR}, 2017.

\bibitem{guo2017calibration}
C. Guo, G. Pleiss, Y. Sun, and K. Q. Weinberger, ``On calibration of modern neural networks,'' in \emph{ICML}, 2017.
\bibitem{metaod_th}
Y. Zhao, R. Rossi, and L. Akoglu, ``Automating outlier detection via meta-learning,'' in \emph{NeurIPS}, 2021.
\bibitem{adbench_th}
S. Han, X. Hu, H. Huang, M. Jiang, and Y. Zhao, ``ADBench: Anomaly detection benchmark,'' in \emph{NeurIPS Datasets and Benchmarks}, 2022.
\bibitem{pyod_th}
Y. Zhao, Z. Nasrullah, and Z. Li, ``PyOD: A Python toolbox for scalable outlier detection,'' \emph{JMLR}, vol.~20, 2019.
\bibitem{wolpert_th}
D. H. Wolpert, ``Stacked generalization,'' \emph{Neural Networks}, vol.~5, no.~2, 1992.
\bibitem{geifman_th}
Y. Geifman and R. El-Yaniv, ``Selective classification for deep neural networks,'' in \emph{NeurIPS}, 2017.
\bibitem{baf_th}
S. Jesus, J. Pombal, D. Alves, et al., ``Turning the tables: Biased, imbalanced, dynamic tabular datasets for ML evaluation (Bank Account Fraud),'' in \emph{NeurIPS Datasets and Benchmarks}, 2022.
\end{thebibliography}

\end{document}
