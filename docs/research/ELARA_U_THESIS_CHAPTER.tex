...
controlled ablations across three regimes and a \emph{sealed natural temporal-shift}
benchmark (D22) resolve RQ3 negatively: the reliability/drift machinery adds no
significant value and is significantly \emph{worse} than plain stacking under
natural shift ($-0.050$ AUROC, CI excl.\ $0$). The honest contribution is a
cross-domain benchmark with a strong stacking baseline and a negative result for
reliability-aware routing---not a sealed universal SOTA claim.
\end{minipage}
\end{titlepage}
