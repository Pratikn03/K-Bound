...
\centering
\vspace*{1.7cm}
{\huge\bfseries ELARA-U: Shift-Aware Reliability Routing for\\ Cross-Domain Anomaly Detection\par}
\vspace{1.4cm}
...
to select, fuse, stack, or fall back without using test labels. The current
evidence supports a strong development-stage claim under injected deployment
degradation: across $123$ tasks from $5$ families, \systemu{} Stack reaches mean
rank $2.54$ versus $3.63$ for stale validation auto-selection, with paired
bootstrap rank delta $-1.10$ and 95\% CI $[-1.54,-0.62]$. It also reduces mean
regret from $0.0454$ to $0.0099$ and lowers the negative-transfer rate versus
rank-mean from the auto-select rate of $0.106$ to $0.098$. The evidence is not
yet a sealed universal SOTA claim because the strongest result is still a
development stress benchmark and additional natural, external, and sealed
...
Rank-normalized stacker & Learns per-task combinations from validation labels and applies monotone-invariant rank features to test scores. & Mean rank $2.54$ stress result \\
...
carry no significant signal beyond validation quality and detector identity.
...
Natural temporal split & Fraud/cyber/time-series train on earlier data, test on later data. & Rank delta vs auto-select CI below zero \\
