% ELARA results: filled tables (emit_tables.py) + data figures (emit_figures.py)
...
benchmark: ELARA's rank-normalized stacking meta-router significantly improves
...
\section{Figures}
\label{sec:figures}
% [fig:rank removed in consolidation: re-plotted the data already in Table tab:main]

% [fig:regret removed in consolidation: re-plotted the data already in Table tab:regret]

% [fig:calib removed in consolidation: re-plotted the data already in Table tab:calib]

\begin{figure}[H]\centering\includegraphics[width=0.55\linewidth]{elara_u_reliability.png}
\caption{Reliability diagram for the isotonic-calibrated ELARA Stack, pooled over 123 tasks (calibrator fit on validation only). Points near the diagonal indicate good calibration; Brier/NLL in Table~\ref{tab:calib}.}\label{fig:reliability}\end{figure}

% [fig:heatmap removed in consolidation: re-plotted the data already in Table tab:family]

% [fig:ablation removed in consolidation: re-plotted the data already in Table tab:ablation]
...
\caption{ELARA architecture: benchmark families feed a detector zoo and score archive; validation-only labels and unlabeled deployment scores drive the router.}\label{fig:arch}\end{figure}
