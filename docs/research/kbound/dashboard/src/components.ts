...
function statusChip(label: string, status?: string): string {
  const dotClass =
    status === "verified" ? "verified" : status === "pending" ? "pending" : "open";
  return `<div class="status-chip"><span class="status-dot status-dot--${dotClass}"></span>${esc(label)}</div>`;
}

export function researchHeader(data: Snapshot): string {
...
export function evidenceStrip(strip: Snapshot["evidence_strip"]): string {
  if (!strip) return "";
...
export function regimeMap(regimes: Snapshot["regime_map"]): string {
  const cards = (regimes ?? [])
    .map(
      (r) => `
...
export function theoryLedger(rows: Snapshot["theory_ledger"]): string {
  const body = (rows ?? [])
    .map(
      (r) => `
      <tr>
...
function policyTableRows(rows?: PolicyRow[]): string {
  return (rows ?? [])
    .map((r) => {
...
function boundaryCard(r: BoundaryRow): string {
  const metrics = [
    ["Freeze", r.freeze],
    ["Adapt", r.adapt],
    ["KGA", r.kga],
...
function edgeUnblockPanel(edge: EdgeValidation): string {
  const u = edge.unblock;
  if (!u) return "";
  const gaps = u.gaps
    .map(
      (g) => `<li class="${g.passed ? "gap-pass" : "gap-fail"}">
...
export function edgeStudyStatus(edge?: EdgeValidation): string {
  if (!edge) return "";
  ...
  return `
    <div class="shadow-diagram" aria-label="Live shadow architecture">
      <svg viewBox="0 0 920 120" width="920" height="120" xmlns="http://www.w3.org/2000/svg" role="img">
        ...
        ${arrow(690, 60, 710, 60)}
        ${arrow(690, 70, 820, 60)}
        <text x="710" y="115" font-size="10" fill="#8C94A3">Commit candidate state only after approval</text>
      </svg>
    </div>`;
}

function box(x: number, y: number, w: number, h: number, label: string, fill: string): string {
...
export function safetyBoundary(safety: Snapshot["safety"]): string {
  if (!safety) return "";
  const metrics = (safety.metrics ?? [])
    .map(
      (m) => `<div class="safety-metric">
...
export function reproducePanel(rep: Snapshot["reproduce"]): string {
  if (!rep) return "";
  return `
    <div class="page-section">
...
export function artifactFooter(meta: Snapshot["meta"], prov: Snapshot["provenance"]): string {
  return `
    <div class="page-section">
      <footer class="artifact-footer">
        <div><strong>Artifact provenance</strong></div>
        <div>Snapshot: <code>${esc(prov?.snapshot_path)}</code> · deterministic manifest build</div>
        <div>Paper: <code>${esc(meta?.paper)}</code> (${esc(meta?.paper_pages)} pp) · build <code>${esc(meta?.build_id)}</code></div>
        <div>Manifest: <code>${esc(prov?.manifest)}</code> · SHA-256 <code>${esc(prov?.manifest_sha256)}</code></div>
...
export function architectureSection(): string {
  return `
    <div class="page-section">
      ...
      <div class="panel panel-pad">
        ...
        <div class="io-grid" style="margin-top:20px">
          <div class="callout">
            <strong>Population layer.</strong> The frontier uses <code>M</code>, <code>gamma</code>, and
            <code>beta</code> to characterize when a strict direction is uniformly supportable over a
            declared target class.
          </div>
          <div class="callout">
            <strong>Empirical layer.</strong> KGA estimates <code>Delta</code> directly from evidence
            <code>Z</code> and commits only when <code>Delta_hat +/- epsilon</code> excludes zero. Real-data
            runs do not numerically apply the population frontier.
          </div>
        </div>
      </div>
    </div>`;
