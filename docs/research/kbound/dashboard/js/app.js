import { architectureSection, artifactFooter, edgeStudyStatus, evidenceBoard, evidenceStrip, experimentsPage, regimeMap, reproducePanel, researchHeader, safetyBoundary, theoryLedger, } from "./components.js";
import { ROUTES, buildPageRenderers, navigate, onRouteChange, parseRoute, routeLabel, } from "./router.js";
import { bindThemeToggle, initTheme } from "./theme.js";
let snapshot = null;
async function loadSnapshot() {
    const res = await fetch("dashboard/data/snapshot.json", { cache: "no-store" });
    if (!res.ok)
        throw new Error(`snapshot.json ${res.status}`);
    return res.json();
}
...
function pageContent(path, data) {
...
function renderShell(data, path) {
...
function bindInteractions(path) {
...
async function bindLiveCameraPreview() {
    const video = document.getElementById("live-camera-preview");
    const select = document.getElementById("live-camera-device");
    const status = document.getElementById("live-camera-status");
    if (!video || !select || !status)
        return;
    const setStatus = (msg) => {
    ...
    const screenBtn = document.getElementById("live-screen-share-btn");
...
function renderRoute(path) {
    ...
    if (title)
        title.textContent = routeLabel(path);
    bindInteractions(path);
