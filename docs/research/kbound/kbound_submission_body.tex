...
Together, these analyses separate two possible obstacles to a justified update: the permitted
evidence may leave its benefit unidentified, or an empirical interval may fail to transfer.
The first requires additional assumptions or information; the second requires evidence for
the calibration procedure at the intended decision unit.
...
The study in \cite{jhawar2026identifiable} independently examines when an unlabeled
observation channel identifies the best TTA action. That September 2026 preprint
...
The construction in \cite{li2026matrixconstraints} obtains finite-sample label-shift
...
\[
 ...
 \end{gathered}}
\]
This equality connects the audit problem to the strict-commitment frontier. For the full
declared class, Theorem~\ref{thm:compact-beta-minimax} rules out a uniformly tighter residual
budget from the same evidence. We therefore treat $\beta$ as an
\emph{externally justified sensitivity parameter}, by analogy to Rosenbaum's sensitivity
...
\subsection{Historical Labels Require a Transfer Assumption}
...
An unhedged point rule ($\operatorname{sign}(\widehat\Delta)$) incurs a $30.9\%$--$35.6\%$ directional error rate ($\{g = \text{ADAPT} \wedge \Delta \le 0\} \cup \{g = \text{FREEZE} \wedge \Delta \ge 0\}$) with $0\%$ abstention (where $\mathrm{FA}_u$ is defined strictly for false \adapt; here directional errors combine false \adapt and false \freeze on null/tie states); crucially, $100\%$ of these errors occur on null/tie states ($Z=0, \Delta=0$) due to estimation noise around zero, with zero errors on strictly harmful states ($\Delta = -0.6$). In the finite-radius configurations, the calibrated cell interval recorded $93.9\%$--$96.5\%$ empirical cell inclusion against a nominal $95\%$ level and zero observed wrong-direction decisions. Observed population inclusion was approximately $100\%$ for the cell interval in this synthetic model. The compound interval also recorded $100\%$ empirical population inclusion and zero observed wrong-direction decisions. Its abstention rate was $99.9\%$--$100\%$ at $m=32$ and approximately one third for the finite-radius configurations with $m\ge128$. At $N_{\mathrm{cal}}=18$, the required calibration rank is unavailable, so the infinite-radius rule abstains throughout. These are finite-simulation observations, not a proof of universal coverage.
...
disagreement, and update size. The current principal CIFAR replay fits a separate
\texttt{GradientBoostingRegressor}~\cite{pedregosa2011sklearn} for each adapter, stream seed and score fold,
using squared-error loss, 250 trees, depth 2, learning rate $0.05$, subsample $0.8$, and model seed 0.
The later shared-predictor comparison has a separately specified estimator configuration
(\KBSuppRef{app:estimator-settings}{Appendix}{the study-specific estimator settings}).
...
The paper distinguishes current outcome-disjoint calibration, Protocol B, from the historical leave-one-cell-out replay, Protocol A.
...
ADAPT/FREEZE exposure. This endpoint evaluates utility retention under the declared protocol; it is not a population-safety guarantee.
...
Figure~\ref{fig:decisive-pareto} illustrates the effect of changing helpful and harmful mixture
weights in a preserved historical Tent seed-0 run. Its constructed mixture curve is separate
from the current Protocol B aggregate above; it is not an additional deployment or a new
cross-fitting evaluation.

\begin{figure}[t]
  \centering
  \kbgraphics[width=\columnwidth]{figures/fig_decisive_pareto_cifar10c.pdf}
  \caption{Historical CIFAR-10-C Tent seed-0 mixture illustration, using the preserved
  \texttt{stress\_grid\_multiseed\_v1/seed0} curve. Mean regret is plotted against the
  constructed harmful fraction $p$ for always-adapt, always-freeze, and KGA. The curve
  illustrates changing mixture weights within that saved run; it does not report the current
  Protocol B aggregate or independently sampled deployments.}
...
These are observed out-of-fold inclusion rates on an opened, dependent benchmark; they do not establish interval coverage on unseen distributions.
...
on the evaluated panel, this rule records 88 nonpositive adaptations ($\mathrm{FA}_u = 0.041$,
$88/2{,}160$). KGA trades point regret for greater directional conservatism: by abstaining on 732 cells whose intervals contain zero, it records zero false ADAPT decisions on these evaluated cells.
...
\centering
\footnotesize
\caption{CCT-20 utility-retention endpoint defined in Eq.~\eqref{eq:cct-safe-utility}.
...
\textbf{A separate retrospective labeled finite-frame audit is reported in Appendix~S. That audit uses a different interval construction and produces 0~ADAPT / 35~FREEZE / 10~ABSTAIN actions; these are audit-assisted decisions and do not revise the primary KGA actions reported here (0~ADAPT / 44~FREEZE / 1~ABSTAIN).}
...
point or margin policies do better in the other cohorts. Within a fixed scoring fold,
subtracting the same finite radius from the same point predictions preserves their ordering.
At matched ADAPT exposure, the constant-radius rule selects the same cells as the
point-ranking control, using the same tie rule. This does not compare rankings across
separately fitted folds or studies; locally varying radii can change the ordering.
The favorable Tent comparison is a point estimate on six observed corruption types.
Omitting JPEG only as an influence diagnostic nearly removes the advantage over point
prediction and reverses the comparison with the tuned margin; all six corruption types
remain in the primary estimate. Paired bootstrap ranges across these types describe
sensitivity, not a population confidence interval or multiplicity-adjusted discovery
(\KBSuppRef{app:journal-empirical}{Appendix}{the supplementary empirical synthesis}).
...
reuses the paired-cancellation row's observable count streams. These archived
outputs retain the earlier numerical qualification described in
\KBSuppRef{app:nextphase-paired-proof}{Appendix}{the supplementary numerical scope}.}
...
certificate would be particularly risky. In high-stakes applications, an unresolved assessment
calls for an application-specific fallback, with domain-expert review where appropriate;
the choice depends on the consequences of continuing with the frozen model.
...
scaled calibration has lower observed CIFAR Tent harm-weighted loss, but the advantage narrows at matched
exposure and depends on the corruption mix. Nominal 90\% constant-cell inclusion falls to $50.51$--$60.69\%$ on held-out CIFAR
...
abstention records the unresolved decision; it does not certify that the retained model is safe.
