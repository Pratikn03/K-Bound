...
Theorem~\ref{thm:compact-beta-minimax} limits what the specified label-free evidence can establish: it cannot uniformly justify a residual bound smaller than the unresolved radius of its evidence fibre. This does not preclude useful guarantees under externally supplied budgets or additional structural assumptions. Proposition~5 gives a direct finite-sample rule under a credible declared budget (\KBSuppRef{app:direct-sampled-frontier}{Appendix}{A.5}). \KGA explores a different route, using historical labeled shift outcomes to predict and calibrate evaluation-cell benefit. Its validity depends on its stated calibration assumptions, rather than on an ability to estimate $\beta$ from deployment observations alone.

\KGA does not attempt to implement the theoretical $\beta$-frontier without historical data; rather, it provides an empirical alternative once historical shift benchmarks are admitted into the evidentiary basis.
...
Our formalization strictly distinguishes population-level guarantees from empirical cell-level estimates. In particular, we formally prove that a label-free one-bit decoder exists if and only if the label is constant on each observable evidence fibre. The stronger claim that one representative per swap orbit suffices is refuted by an explicit machine-checked counterexample. Furthermore, rate transfer holds only under an explicit episode-level contract ($\mathrm{miss}_{T}\le H\,\mathrm{miss}_{S}$, $H\ge0$, and $\mathrm{miss}_{S}\le\alpha$); it does not estimate $H$, prove benchmark exchangeability, or establish prospective population provenance. Thus K-Bound's population interval $(M,\gamma,\beta)$ and KGA's fitted cell interval $(\widehat\Delta,\varepsilon)$ remain mathematically distinct, with the population procedure incorporating the separately justified sampling radius $b(m,\delta)$.
...
A retrospective 72-condition Camelyon17 diagnostic (\KBSuppRef{app:camelyon17}{Appendix}{B.5}) contains both helpful and harmful adaptation cells under hospital-provenance shift; under authenticated outcome-blind execution, the gate issued 0 \adapt, 2 \freeze, and 70 \abstain decisions, matching always-freeze regret ($0.0438$) and demonstrating conservative retention without \adapt exposure (alongside an exploratory 31-\adapt post-hoc sensitivity partition reported in Appendix B.5). Across the evaluated conservative natural panels (CCT-20 camera traps, the 72-condition Camelyon17 diagnostic under authenticated execution, and RxRx1), KGA makes zero opportunistic \adapt decisions, evaluating conservative retention rather than selective routing. Transferring a fixed benefit predictor across completely unseen wild domains without target calibration remains an open empirical challenge.
...
\paragraph{Ablation: Empirical surrogate for $\beta$.}
...
This operational failure directly corroborates Theorem~\ref{thm:compact-beta-minimax}: processing unchanged evidence
cannot replace a justified structural assumption.
...
\begin{enumerate}
  ...
  in that study. However, it made zero \adapt decisions. Prospectively demonstrating selective opportunistic routing on an unopened natural-shift target remains an open empirical challenge. The retrospective Camelyon17 diagnostic in \KBSuppRef{app:camelyon17}{Appendix}{B.5} similarly exhibits conservative retention (zero \adapt decisions under authenticated outcome-blind execution) and does not establish opportunistic routing.
  ...
  internal benefit estimators on common saved features. An authenticated head-to-head comparison with the authenticated native AETTA accuracy estimator with an explicitly specified recovery rule is reported on the multi-model mixed-sign panel in \KBSuppRef{app:baseline-audit}{Appendix}{the supplementary baseline audit}. However, official implementations for certain external baselines (e.g., Kim et al.'s TTA/ALine~\cite{kim2024ttaline}, Baek et al.'s ALine~\cite{baek2022aol}, and original POEM pipelines) were deprecated, partially incomplete, or incompatible with our standardized multi-platform evaluation harness. We therefore evaluate them via our faithfully reconstructed component ports on common feature representations.
\end{enumerate}
...
Empirically, we find that enforcing rigorous finite-sample safety bounds frequently results in conservative retention of the frozen model. This ``price of rigor'' highlights a fundamental tension in test-time adaptation: opportunistic routing on unseen domains is inherently risky, and mathematical safety naturally suppresses adaptation at standard batch sizes.
