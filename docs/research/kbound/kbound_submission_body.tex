...
Historical TTA/ALine, AETTA and POEM port labels do not establish faithful
implementations of the published estimators or controllers. Authenticated native
comparisons under a shared protocol remain distinct from this saved-feature diagnostic.
...
cells, not an estimate of the latent population residual $\beta$. Furthermore, historical data do
not automatically establish calibration transfer to unseen deployment environments.
...
Proposition~\ref{thm:population-transfer} formalizes the conditional bridge to population risk $\Delta$: under independent fresh evaluation draws, adding the Hoeffding concentration radius $b(m,\delta)$ to the empirical radius $\varepsilon$ yields a compound population interval with radius $\varepsilon+b$ and error budget $\alpha_{\mathrm{cell}}+\delta$. The full protocol, counterexample, and proof are detailed in \KBSuppRef{app:population-transfer}{Appendix}{the population-transfer supplement}. No such sampling-error bound is established for the reported benchmark panels; in particular, when a candidate adapts to evaluation inputs, transductive normalization couples the candidate across batches.
...
Carlo error but does not verify this null model. Retrospective Holm adjustment over the six prospectively named contrasts requires valid input $p$-values;
...
$\CIFAReataAdapt$, and $\CIFAReataFreeze$. In the canonical primary panel (Table~\ref{tab:primary-cifar}), their actions are 1,107/359/694 and
1,241/132/787, respectively; both have zero observed false adaptations ($\mathrm{FA}_u = 0.0000$).
SAR commits 1,446/0/714 actions with regret $0.0016$, versus $0.0003$ for always-adapt (and $0$ false adaptations).
...
freezing and abstention to achieve net regret reductions over always-adapt ($+0.0064$ for Tent,
$+0.0020$ for EATA on the canonical primary panel; or $+0.0062$ and $+0.0017$ in the synchronized baseline study).
Figure~\ref{fig:decisive-pareto} illustrates this regret-minimization behavior across deployment streams with varying harmful fraction $p$: while unconstrained always-adapt degrades linearly as harmful mass increases, KGA tracks the lower Pareto envelope, preserving baseline safety without forfeiting adaptation gains when helpful.

\begin{figure}[t]
  \centering
  \kbgraphics[width=\columnwidth]{figures/fig_decisive_pareto_cifar10c.pdf}
  \caption{CIFAR-10-C stress grid mixing-ratio Pareto front: mean regret vs.\ the harmful fraction $p$ of the deployment stream for Tent, comparing always-adapt, always-freeze, and KGA. While unconstrained always-adapt degrades sharply as the harmful fraction increases and always-freeze stalls on helpful distributions, KGA tracks the lower Pareto envelope, maintaining low regret across all mixture proportions and preventing catastrophic model failure.}
...
$88/2{,}160$). KGA explicitly trades this regret gap for directional safety: by withholding
commitment on 732 ambiguous cells, it eliminates false adaptations entirely on these records.
...
Detailed exact-replay receipts are indexed in \KBSuppRef{sec:compact-repro}{Appendix}{the supplementary reproducibility ledger}.
...
KGA matched always-freeze exactly and avoided the measured degradation of always-adapt (\CCTSafeUtilityPass),
earning the recorded verdict \CCTVerdict. However, the stronger opportunistic routing criterion was not met:
...
unseen environments; selective opportunistic routing under natural shift remains an open empirical challenge.
...
The opened Camelyon17 panel shows that KGA can issue ADAPT decisions on natural data;
it does not establish why CCT-20 produced no such decisions. On the observed CCT-20 cells,
KGA retains the frozen model, whereas the one-sided Camelyon17 panel retains adaptation
exposure. These candidate-specific outcomes do not demonstrate prospective selective routing.
Transferring a fixed benefit predictor
across completely unseen wild domains without target calibration remains an open empirical challenge.
...
\centering
\small
\caption{Exploratory parameter-restricted adaptation probe on CCT-20 camera-trap deployments. Safe retention denominator is 45/45 evaluated cells (9 target locations $\times$ 5 checkpoints) serving the frozen baseline. The break-even margin $\beta^*$ indicates the local empirical tolerance for overturning the decision, not population truth.}
\label{tab:cct20-partial-adaptation}
\resizebox{\textwidth}{!}{\input{paper/generated/tab_cct20_partial_adaptation.tex}}
\end{table*}

On the 45 CCT-20 evaluation cells, unconstrained full adaptation harms accuracy on 44 of 45 cells. KGA achieves 100\% safe retention by serving the frozen model across all 45 cells (44 FREEZE, 1 ABSTAIN, 0 ADAPT), avoiding measured harm. In exploratory development probes restricting updates to normalization statistics or prediction heads, drift is dampened ($\beta^* = 0.013$--$0.022$, where $\beta^*$ denotes the local empirical break-even margin). However, within the maintained benchmark, all 45 cells retain the frozen baseline; these exploratory probes remain development diagnostics and do not demonstrate active opportunistic adaptation on unseen natural shifts.
...
\begin{enumerate}
  ...
  in that study. However, it made zero \adapt decisions. Demonstrating selective, opportunistic routing that
  identifies and exploits beneficial adaptation under natural shifts remains an open empirical challenge.
  ...
  \item \textbf{Single-Checkpoint Dependence:} The 2,160 CIFAR-10-C cells reuse a single pre-trained
  ResNet-18 source checkpoint across corruption conditions. These evaluate 2,160 distinct adaptation conditions,
  not 2,160 independent models. Multi-checkpoint evaluations across independently trained source models remain
  future work.
  ...
  internal benefit estimators on common saved features. While internal components and scoped native runs have been
  verified across CPU and Apple Silicon (MPS) environments, full execution of external baseline drivers is
  constrained by documented method-specific dependencies: the authors provide code for Kim et al.'s TTA/ALine~\cite{kim2024ttaline}; our current scored panel, however, contains no authenticated outputs from that implementation (0/15 available), reflecting an incomplete local evaluation rather than an absence of published code. Baek et al.'s ALine~\cite{baek2022aol} has partial component availability (11/15 available),
  and original POEM/AETTA pipelines require upstream external library versions and native CUDA configurations.
\end{enumerate}
