...
K-Bound and Knowability-Guided Adaptation (\KGA) address the same update decision using different information. K-Bound characterizes which strict benefit signs are supported by fixed label-free evidence and a declared residual bound in a binary model. \KGA uses labeled historical shift outcomes to fit and calibrate an empirical benefit gate. It is an empirical companion, not an implementation of the population frontier. Section~\ref{sec:compact-bridge} explains the relationship and the additional assumptions needed to connect evaluation-cell benefit to population benefit.
...
Two additional evaluations test the limits of these contributions. A paired-prediction
transport certificate gives a concrete finite-sample decision procedure under an explicit
conditional-shift assumption, using established confidence-region and LP machinery.
A shared-predictor comparison isolates what residual calibration adds beyond point prediction
and a tune-selected margin. Its favorable and unfavorable cases are reported together in
Section~\ref{sec:nextphase-comparisons}; neither evaluation establishes general deployment safety.
...
implementations of the published estimators or controllers. A historical comparison against the native AETTA accuracy estimator with an explicitly specified recovery rule is reported in \KBSuppRef{app:baseline-audit}{Appendix}{the supplementary baseline audit}. Recovered mixed-sign per-cell decisions and B6 fold receipts support the saved arithmetic; original fitted-model/environment identities remain incomplete, and historical B6 rank clipping prevents nominal coverage promotion (\KBSuppRef{app:replication-provenance}{Appendix}{the supplementary replication-provenance qualification}); native-estimator identity alone does not authenticate the whole comparison. Full POEM and native TTA/ALine remain separate from this saved-feature diagnostic.
...
a historical AETTA comparison is reported in \KBSuppRef{app:baseline-audit}{Appendix}{the supplementary baseline audit}, with recovered mixed-sign KGA receipts and a rank-clipped calibration caveat, while native TTA/ALine and full POEM comparisons remain incomplete.}
...
Theorem~\ref{thm:compact-beta-minimax} shows why label-free deployment observations cannot audit
their own calibration residual bound without additional structure. An empirical decision layer must
therefore draw upon external information or domain assumptions, such as the routes cataloged in
Section~\ref{sec:compact-additional-structure}. KGA explores the historical-shift-record route,
using labeled prior shifts to fit and calibrate an empirical benefit gate.
...
Section~\ref{sec:compact-bridge} details this architecture, its validity conditions, and the
distinction between evaluation-cell predictions and population guarantees.
...
whole procedure is not one extra forward pass. Historical runtime literals, including the $0.62\,\text{ms}$ combined gate overhead, are preserved in \KBSuppRef{tab:system-latency-profile}{Table}{the supplementary system latency table} for auditability. No matching synchronized timing samples or peak-memory logs were recovered, so these values are not authenticated performance measurements. Comparable end-to-end deployment latencies across the historical benchmark tracks remain unestablished.
...
All recorded development cells for Tent had negative measured benefit. On the evaluated CCT-20 panel, KGA issued no ADAPT decisions. This result demonstrates frozen-model retention, not successful selection of beneficial updates.
...
Two historical probes require a distinction from measured benchmarks. The recovered four-architecture producer samples synthetic paired outcomes using architecture-labeled constants; it does not execute those backbones. Table~\ref{tab:multi-architecture-invariance} now reports only saved simulation summaries. The separate ImageNet-R architecture experiment retains its own measured results and limitations.
...
The parameter-restricted probe also samples synthetic benefits at five hardcoded locations rather than loading CCT-20 images or executing the proposed adapters. Its numerical break-even summaries are retained as simulation outputs in Table~\ref{tab:cct20-partial-adaptation}; the saved null actions do not support a routing result.
...
Separately, the authenticated CCT-20 benchmark retained the frozen model across all 45 cells (44 FREEZE, 1 ABSTAIN, 0 ADAPT). That safe-utility result does not establish positive selective adaptation or validate any of the simulated restricted-adapter regimes.

\section{Additional Tests of Decision Value}
...
\resizebox{\textwidth}{!}{\input{paper/generated/tab_nextphase_calibration.tex}}
\end{table*}
...
CIFAR benefits; scaling and group maxima do not restore nominal inclusion.
...
\resizebox{\textwidth}{!}{\input{paper/generated/tab_nextphase_paired_transport.tex}}
\end{table*}
...
\input{next_phase/field_study_results.tex}
...
An earlier draft reported an empirical benefit-scale proxy as a surrogate for $\beta$. The raw sweep artifact has not been recovered, so the previously reported numerical failure ranges remain unverified historical summaries, consistently with the supplementary provenance note. They cannot empirically corroborate Theorem~\ref{thm:compact-beta-minimax}. That theorem independently establishes a conditional identification obstruction: processing unchanged evidence cannot replace a justified structural assumption.
The theorem does not attribute this failure to any single cause. Rather,
Theorem~\ref{thm:compact-beta-minimax} proves that $\beta$ cannot be audited from label-free target
data alone within this rich kernel class. Consequently, population guarantees within this framework without coupled target labels
or alternative structural assumptions (such as those discussed in Section~\ref{sec:compact-additional-structure}) require
treating $\beta$ as an externally declared sensitivity parameter (analogous to Rosenbaum sensitivity bounds in causal inference), whereas
KGA pursues empirical evaluation-cell interval coverage using historical shift benchmarks.
...
The shared-predictor comparison finds a calibration advantage for one retrospective
CIFAR Tent cohort, alongside losses to simple margins and substantial held-out
coverage failure. A paired-transport extension demonstrates how joint prediction
information can identify a beneficial decision under explicit assumptions, and
how the identical observable evidence supports harmful decisions when those
assumptions fail. These results narrow the supported practical claim rather than
...
The empirical evidence shows a candidate-dependent utility--commitment trade-off, not a universal cost of mathematical safety. In the reported population-transfer sensitivity calculation, adding the stated concentration radius reduces adaptation exposure; that calculation is a sufficient bound under its sampling assumptions, not a necessary sample-size threshold or an explanation of every historical abstention. Conservative retention alone establishes neither useful selective routing nor valid protection on unseen environments.
