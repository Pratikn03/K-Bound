...
Because all development cells for Tent were harmful, the calibration distribution supplied no positive examples, leading the
model to predict negative benefit across all evaluation cells.
This establishes effective conservative retention of the frozen model on this camera-trap deployment, while illustrating that selective routing requires exposure to positive adaptation signals during calibration.
...
A retrospective 72-condition Camelyon17 diagnostic (\KBSuppRef{app:camelyon17}{Appendix}{B.5}) contains both helpful and harmful adaptation cells under hospital-provenance shift; under authenticated outcome-blind execution, the gate issued 0 \adapt, 2 \freeze, and 70 \abstain decisions, matching always-freeze regret ($0.0438$) and demonstrating conservative retention without \adapt exposure (alongside an exploratory 31-\adapt post-hoc sensitivity partition reported in Appendix B.5). Across all evaluated natural panels (CCT-20 camera traps, the 72-condition Camelyon17 diagnostic, and RxRx1), KGA makes zero \adapt decisions, reliably defaulting to conservative retention when domain shift degrades predictive certainty.
...
Across the four topologies, the disagreement region measure $\mu(D)$ ($0.1727$--$0.1806$) and the calibrated certificate radius $r = \varepsilon + b$ ($0.1023$--$0.1026$) exhibit comparable scale on these evaluation batches. However, similar radii do not establish empirical coverage, which requires held-out inclusion testing. On these severe corruptions where unhedged adaptation collapses, the gate safely halts adaptation by issuing exclusively FREEZE or ABSTAIN actions ($\mathrm{FA}_u = 0.0000$).
...
On the 45 CCT-20 evaluation cells, unconstrained full adaptation harms accuracy on 44 of 45 cells. KGA served the frozen model across all 45 cells (100\% frozen-model retention: 44 FREEZE, 1 ABSTAIN, 0 ADAPT), avoiding the measured adaptation degradation. In exploratory development probes restricting updates to normalization statistics or prediction heads, drift is dampened ($\beta^* = 0.013$--$0.022$, where $\beta^*$ denotes the local empirical break-even margin), though within the maintained benchmark, all 45 cells consistently retain the frozen baseline.
