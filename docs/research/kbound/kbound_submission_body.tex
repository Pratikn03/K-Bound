...
implementations of the published estimators or controllers. A historical comparison against the native AETTA accuracy estimator with an explicitly specified recovery rule is reported in \KBSuppRef{app:baseline-audit}{Appendix}{the supplementary baseline audit}. Its KGA per-cell decision and fitting/calibration receipts have not been recovered (\KBSuppRef{app:replication-provenance}{Appendix}{the supplementary replication-provenance qualification}); native-estimator identity alone does not authenticate the whole comparison. Full POEM and native TTA/ALine remain separate from this saved-feature diagnostic.
...
a historical AETTA comparison is reported in \KBSuppRef{app:baseline-audit}{Appendix}{the supplementary baseline audit}, subject to missing KGA execution receipts, while native TTA/ALine and full POEM comparisons remain incomplete.}
...
Across the four topologies, the disagreement region measure $\mu(D)$ ($0.1727$--$0.1806$) and the calibrated certificate radius $r = \varepsilon + b$ ($0.1023$--$0.1026$) exhibit comparable scale on these evaluation batches. However, similar radii do not establish empirical coverage, which requires held-out inclusion testing. In this probe, KGA issued only FREEZE or ABSTAIN decisions and therefore retained the frozen predictor throughout. The zero observed false-ADAPT rate reflects the absence of ADAPT decisions, not successful selection of beneficial updates.
