...
harmful+helpful regimes test whether the controller adapts, freezes, and abstains in the corresponding conditions, including a pre-registered
...
datasets this mostly acts as safety or abstention. Improvement over both fixed policies requires a detectable mixed regime but is not the primary objective;
...
\item \textbf{Regime-based empirical evaluation.} A uniform panel evaluates action validity, false adaptation, coverage, natural no-harm behavior, constructed routing, and weak-evidence abstention; fixed-policy regret comparisons are secondary.
\end{enumerate}
...
Mixed + detectable & CIFAR-10-C Tent/EATA, ImageNet-C SAR & adapt helpful, freeze harmful, abstain near zero & low false-adapt; CI-supported utility \\
Weak evidence & ImageNet-R, CIFAR-10.1 & abstain / conservative & diagnostic; no promoted win \\
Constructed mixture & three-source OOF stream & route per condition & supported routing result, not transfer \\
\bottomrule
\end{tabular}
}
\end{table}
...
\textbf{Thm.~\ref{lem:nonid}} (opposite-world construction) $\rightarrow$
\textbf{Cor.~\ref{cor:matched-abstain}} (abstention lower bound) \\[2pt]
...
regimes exercise all three actions and make routing utility observable. Fixed-policy regret is secondary. This is a \emph{conceptual schematic}: axis positions are illustrative regime
...
The previously mismatched SAR aggregate was rebuilt from all five saved per-condition seed files
using the locked Protocol~A analysis. The rebuild yields regret $0.0015/0.0112/0.1286$ for
KGA/always-adapt/always-freeze and false-adapt $0/2160$; paired condition-bootstrap intervals
exclude zero against both fixed policies. This is retained as a current-tree reconciliation rather
than evidence that every historical archived SAR summary was equivalent: the SAR conformal radius
is substantially less stable across seeds (CV $0.390$) than Tent or EATA.
...
reaches $0.427$, \textbf{beating both} trivial policies (regret $0.0264$ vs $0.0529$/$0.0319$)
at $\mathrm{FA}_{\mathrm u}=0.000$, using the exact split-conformal radius $\varepsilon=\rho_{(k)}$,
$k{=}\lceil(n{+}1)(1-\alpha)\rceil$. The pooled $95\%$ paired-bootstrap gap to always-freeze is
$[-0.0088,-0.0027]$ and to always-adapt $[-0.052,-0.003]$; both intervals exclude zero.
...
SAR  & always-adapt     & $0.385$          & $0.0529$ & $16\%$ & \\
     ...
     & \textbf{K-Bound} & $\mathbf{0.427}$ & $\mathbf{0.0264}$ & & \textbf{yes} \\
\bottomrule
\end{tabular}
}
...
KGA / always-adapt / always-freeze; lower is better. Fixed-policy comparisons are secondary. The final
...
\midrule
CIFAR-10-C stress & $5\times432$ per candidate & Tent: $0.0016/0.0080/0.1239$; EATA: $0.0013/0.0033/0.1313$; SAR withheld; $\mathrm{FA}_{\mathrm u}=0$ & selective routing; paired-CI utility & Tent/EATA reconciled from five raw seeds; SAR withheld (seed-0 aggregate non-reproducing) \\
ImageNet-C SAR & $27$ cells $\times$ $5$ seeds & $0.0264/0.0529/0.0319$ pooled; $\mathrm{FA}_{\mathrm u}=0.000$ (exact split-conformal) & selective routing; pooled CI-supported utility; point-level $5/5$ seeds & locked (5 seeds; per-seed CIs $2/5$) \\
Camelyon17 OOD & genuine OOD test $n=18$ (support $n=36$) & $0.0000/0.0000/0.1381$; $\mathrm{FA}_{\mathrm u}=0$ & selects adapt-equivalent behavior & reconciled (raw records) \\
iWildCam H v2 & held-out test $n=72$ & $0.0041/0.1028/0.0041$; $\mathrm{FA}_{\mathrm u}=0$ & no-harm, ties freeze & reconciled (OOF lock; 5-seed real-ckpt confirmed) \\
Office-Home M v2 & target test $n=35$ & $0.0157/0.0468/0.0158$; $\mathrm{FA}_{\mathrm u}=0$ & no-harm, tiny point edge & reconciled (OOF lock; 5-seed confirmed) \\
RxRx1 J & OOD test $n=60$ & $0.0000/0.2587/0.0000$; $\mathrm{FA}_{\mathrm u}=0$ & selects freeze-equivalent behavior & locked (5 seeds, real ckpt) \\
PACS & four LODO targets, $108$ cells each & three safety targets ($\mathrm{FA}_{\mathrm u}{=}0$); one null (photo, $\mathrm{FA}_{\mathrm u}{=}0.056$) & action-safety diagnostic & diagnostic (1 of 3 planned seeds) \\
ImageNet-R D & $3$ seeds $\times10$ backbones & no CI-robust routing utility; one point-only arm fails the second CI & abstention / fixed-policy ties & diagnostic (3 of 4 planned seeds) \\
...
\midrule
CIFAR-10-C Tent & $5\times432$ & .0016 & .0079 & .1241 & locked; selective routing, $\mathrm{FA}_{\mathrm u}=0$; CI utility \\
CIFAR-10-C EATA & $5\times432$ & .0013 & .0033 & .1314 & locked; selective routing, $\mathrm{FA}_{\mathrm u}=0$; CI utility \\
ImageNet-C SAR & $27$ cells $\times5$ seeds & .0264 & .0529 & .0319 & locked (5 seeds); pooled CI routing utility \\
Camelyon17 OOD & $n=18$ & .0000 & .0000 & .1381 & reconciled; no-harm \\
RxRx1 J & $n=60$ & .0000 & .2587 & .0000 & locked (5 seeds); no-harm \\
...
completed breadth tracks, not universal task-performance claims. The three-source mixture is a constructed
...
not formalize the target-law witness construction, equality of its induced evidence laws, membership
of those laws in $\mathcal C_\beta$, the zero-versus-strict boundary construction, frontier
necessity or maximality, risk alignment, multiclass identifiability, calibration transfer, or a
...
CIFAR-10-C mixed-regime action validity & empirical & Five-seed raw decisions, action rates, false-adapt, and paired CIs & Table~\ref{tab:decisive}; empirical audit manifest & SAR rebuilt from current raw tree; higher radius instability disclosed. \\
