...
\midrule
Helpful-dominated & Camelyon17 OOD & tie always-adapt & locked no-harm \\
Harmful-dominated & RxRx1; iWildCam/Office-Home (OOF lock) & tie always-freeze, beat always-adapt & locked no-harm \\
Domain-gen.\ natural & PACS (4 LODO splits) & no-harm or abstain & diagnostic (1/3 seeds); photo null unreconciled \\
Mixed + detectable & CIFAR-10-C Tent/EATA, ImageNet-C SAR & adapt helpful, freeze harmful, abstain near zero & locked; low false-adapt; CI-supported utility \\
Weak evidence & ImageNet-R, CIFAR-10.1 & abstain / conservative & diagnostic; CIFAR-10.1 locked fail; ImageNet-R incomplete \\
...
\midrule
CIFAR-10-C stress & $5\times432$ per candidate & Tent: $0.0016/0.0079/0.1241$; EATA: $0.0013/0.0033/0.1314$; SAR withheld; $\mathrm{FA}_{\mathrm u}=0$ & selective routing; paired-CI utility & locked (Tent/EATA; five raw seeds); SAR withheld (seed-0 aggregate non-reproducing) \\
...
Camelyon17 OOD & genuine OOD test $n=18$ (support $n=36$) & $0.0000/0.0000/0.1381$; $\mathrm{FA}_{\mathrm u}=0$ & selects adapt-equivalent behavior & locked (OOD reconciliation seal) \\
iWildCam H v2 & held-out test $n=72$ & $0.0041/0.1028/0.0041$; $\mathrm{FA}_{\mathrm u}=0$ & no-harm, ties freeze & locked (OOF bootstrap; single-run) \\
Office-Home M v2 & target test $n=35$ & $0.0157/0.0468/0.0158$; $\mathrm{FA}_{\mathrm u}=0$ & no-harm, tiny point edge & locked (OOF no-harm only; LOO BB not promoted) \\
RxRx1 J & OOD test $n=60$ & $0.0000/0.2531/0.0000$; $\mathrm{FA}_{\mathrm u}=0$ & selects freeze-equivalent behavior & locked (real ckpt; single-run) \\
PACS & four LODO targets, $108$ cells each & primary replay $\mathrm{FA}_{\mathrm u}{=}0$ on all four targets; earlier photo-null ($0.056$) not reproduced & action-safety diagnostic & diagnostic (1 of 3 seeds; photo unreconciled) \\
...
CIFAR-10.1 K & cross-seed test $n=48$ & $0.0021/0.0190/0.0017$; $\mathrm{FA}_{\mathrm u}=0.167$, $\mathrm{FA}_{\mathrm c}=0.444$ & fails transfer bar; no claim & locked diagnostic fail \\
\bottomrule
\end{tabular}
\end{table*}
...
genuine OOD and RxRx1 are locked no-harm results: KGA ties the better fixed policy at
zero observed false-adapt. Office-Home and iWildCam are locked from their out-of-fold bootstrap seal
(App.~\ref{app:claim-artifact}; Office-Home promotes OOF no-harm only; LOO beats-both is not claimed).
PACS and ImageNet-R remain incomplete diagnostics; CIFAR-10.1 is a locked diagnostic fail.
