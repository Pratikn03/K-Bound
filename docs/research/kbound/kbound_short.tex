...
\subsection{ImageNet-C SAR (mechanism-faithful, five seeds)}
...
$0$--$4$, full-scale), SAR is harmful on $15.6\%$ of cell--seed pairs under a
mechanism-faithful re-implementation (SAM, recovery reset, EMA) at the learning rate
shared across candidates. Pooled over five seeds (seed-averaged conditions, the same paired
design as the CIFAR-10-C rows), always-adapt drops to $0.385$ accuracy while \textsc{KGA}
reaches $0.427$, \textbf{beating both} trivial policies (regret $0.0107$ vs $0.0529$/$0.0319$)
at $\mathrm{FA}_{\mathrm u}=0.007$. The pooled $95\%$ paired-bootstrap gap to always-freeze is
$[-0.033,-0.009]$ and to always-adapt $[-0.090,-0.003]$; both intervals exclude zero.
Per seed, the point-level beats-both verdict holds on $5/5$ seeds; the per-seed $27$-cell CIs
exclude zero on $2/5$ (seeds $0$--$1$) and narrowly include it on the lighter-harmful-mass seeds
(App.~\ref{app:imagenetc-ms})---the win scales with detectable harmful mass, as the regime map
predicts. EATA stays helpful-dominated
(\textsc{KGA} ties always-adapt); on Tent the harmful regime dominates ($55.6\%$ of cell--seed
pairs) and \textsc{KGA} abstains to always-freeze (Table~\ref{tab:imagenetc-faithful}). Under the
...
Tent & always-adapt     & $0.402$          & $0.0191$ & $56\%$ & \\
     ...
     & \textbf{K-Bound} & $\mathbf{0.407}$ & $\mathbf{0.0139}$ & & no (${\approx}$ties freeze) \\
\midrule
EATA & always-adapt     & $\mathbf{0.440}$ & $0.0001$ & $2\%$  & no (ties adapt) \\
     & always-freeze    & $0.406$          & $0.0342$ &        & \\
     & \textbf{K-Bound} & $\mathbf{0.440}$ & $0.0003$ &        & \\
\midrule
SAR  & always-adapt     & $0.385$          & $0.0529$ & $16\%$ & \\
     & always-freeze    & $0.406$          & $0.0319$ &        & \\
     & \textbf{K-Bound} & $\mathbf{0.427}$ & $\mathbf{0.0107}$ & & \textbf{yes} \\
\bottomrule
\end{tabular}
}
\caption{ImageNet-C, mechanism-faithful SAR ($27$ cells: 3 noise corruptions, ResNet-50,
$\alpha{=}0.1$); pooled over seeds $0$--$4$ (seed-averaged conditions), superseding the earlier
single-seed and 36-cell provisional configurations. Per-seed detail in
...
ImageNet-C SAR & $27$ cells $\times$ $5$ seeds & $0.0107/0.0529/0.0319$ pooled; $\mathrm{FA}_{\mathrm u}=0.007$ & \textbf{paired-bootstrap BB (pooled)}; point-level $5/5$ seeds & locked (5 seeds; per-seed CIs $2/5$) \\
...
ImageNet-C SAR & $27$ cells $\times5$ seeds & .0107 & .0529 & .0319 & locked (5 seeds); pooled paired-bootstrap BB \\
...
ImageNet-C SAR paired-bootstrap beats-both & locked (5 seeds) & 27-cell noise grid $\times$ 5 seeds; pooled paired CIs exclude 0; point-level $5/5$ \\
...
but no external-lab replication; more seeds would strengthen PACS. Abstention lowers \emph{decision} coverage
...
ImageNet-C authoritative result & empirical & 27 cells $\times$ 5 seeds, SAR candidate, pooled seed-averaged paired bootstrap & Table~\ref{tab:imagenetc-faithful}; pooled bootstrap JSON & Per-seed CIs exclude 0 on $2/5$ at $n{=}27$; point-level $5/5$. \\
