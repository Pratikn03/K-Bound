...
%  (matched rate n ASYMP (sigma^2/Delta^2) log(1/alpha); the deployed CERTIFICATE
%  sits at a universal-constant factor (-> 16 as alpha -> 0) above the two-point
%  lower bound, and the ABSTRACT optimal LRT test sits at -> 3.3-3.6 above it
%  (Bretagnolle-Huber looseness)).  Tight constants are NOT claimed (separate open problem).
%
...
\begin{align*}
\mathrm{FA}(g)&=\Pr_{P_-}(g=\textsc{adapt}), &
\mathrm{FF}(g)&=\Pr_{P_+}(g=\textsc{freeze}),\\
\mathrm{miss}_+(g)&=\Pr_{P_+}(g\neq\textsc{adapt}), &
\mathrm{miss}_-(g)&=\Pr_{P_-}(g\neq\textsc{freeze}).
\end{align*}
...
$\varepsilon_n<\Delta/2$, the certificate commits to the correct sign with probability $\ge1-\alpha$
...
\[
   ...
   \;\le\;
   n_{\mathrm{cert}}\;\le\;
   \frac{8\sigma^2}{\Delta^2}\,\log\!\frac{2}{\alpha}\;+\;1 .
   \;}
\]
Consequently
\[
...
attains this rate: it is \emph{minimax order-optimal} on the identifiable side. The certificate's
certifying sample complexity $n_{\mathrm{cert}}\le\tfrac{8\sigma^2}{\Delta^2}\log(2/\alpha)+1$
sits at a universal-constant factor above the two-point lower bound, the ratio of these leading
constants tending to $16$ as $\alpha\to0$. (The \emph{abstract} optimal likelihood-ratio test sits
at a smaller universal factor, $n_{\mathrm{opt}}/n_{\mathrm{LB}}\to3.3$--$3.6$, the genuine
Bretagnolle--Huber looseness; the certificate's further $\approx4\times$ over $n_{\mathrm{opt}}$ is
the abstain-band penalty --- see Remark~\ref{rmk:opt-delicate}.) The lower bound holds
...
factor (the certificate-to-lower-bound constant $\to16$, decomposing as the $\to3.3$--$3.6$
Bretagnolle--Huber looseness of the optimal test times the certificate's $\approx4\times$
abstain-band penalty) is the separately-listed tight-constants open problem
(\texttt{knowability\_rates.tex}, \texttt{onebit\_audit\_rate.tex}, and
\texttt{theory\_v2/wave3\_tight\_and\_computability.tex}).
...
(sub-Gaussian concentration; the Maurer--Pontil empirical-Bernstein radius
self-normalizes to the same leading term and adds only a
lower-order $O\!\big(R\log(1/\alpha)/n\big)$ range term). The certifying condition is the
\emph{containment} of the $(1-\alpha)$ ball $\{|\bar X_n-\Delta|\le\varepsilon_n\}$ in the
commit region $\{\bar X_n-\varepsilon_n>0\}$ (world $P_+$). At the worst point of that ball,
$\bar X_n=\Delta-\varepsilon_n$, the commit margin is $\bar X_n-\varepsilon_n=\Delta-2\varepsilon_n$,
so containment holds iff $\Delta-2\varepsilon_n>0$, i.e.\ $\varepsilon_n<\Delta/2$ --- \emph{not}
$\varepsilon_n<\Delta$. (At $\varepsilon_n=\Delta$ the worst-case mean $\bar X_n=0$ has commit
probability $\Phi(0)=\tfrac12$, far above $\alpha$; the abstain band has full width $2\varepsilon_n$,
so the safe distance from the boundary is $\varepsilon_n$, which the margin $\Delta$ must exceed by
a further $\varepsilon_n$.) Whenever $\varepsilon_n(\alpha)<\Delta/2$, i.e.\
\[
   \sigma\sqrt{\tfrac{2\log(2/\alpha)}{n}}<\frac{\Delta}{2}
   \quad\Longleftrightarrow\quad
   n>\frac{8\sigma^2}{\Delta^2}\log\frac2\alpha ,
\]
...
$\alpha$-certifying once $n>\tfrac{8\sigma^2}{\Delta^2}\log(2/\alpha)$, which gives its certifying
sample complexity $n_{\mathrm{cert}}\le\tfrac{8\sigma^2}{\Delta^2}\log(2/\alpha)+1$, hence
$n^\star\le n_{\mathrm{cert}}\le\tfrac{8\sigma^2}{\Delta^2}\log(2/\alpha)+1$. (The bounded-benefit
version is identical with $\sigma^2=1-\Delta^2$ and the extra $O(R/\Delta)$ range term, which is
...
\medskip
\noindent Combining the two displays yields the boxed sandwich, and the ratio of the
certificate's leading constant to the lower bound's is
$\big(\tfrac{8\sigma^2}{\Delta^2}\log\tfrac2\alpha\big)\big/\big(\tfrac{\sigma^2}{2
\Delta^2}\log\tfrac1{4\alpha}\big)=16\cdot\log(2/\alpha)/\log(1/(4\alpha))\to16$ as $\alpha\to0$.
(The \emph{abstract} optimal LRT frontier $n_{\mathrm{opt}}=(\sigma/\Delta)^2 z_{1-\alpha}^2$ sits at
the smaller ratio $n_{\mathrm{opt}}/n_{\mathrm{LB}}\to3.3$--$3.6$; the certificate's extra
$\approx4\times$ over $n_{\mathrm{opt}}$ is the abstain-band penalty, i.e.\ the $2\times$ in
$\varepsilon$ from requiring $\varepsilon_n<\Delta/2$.) This proves Theorem~\ref{thm:minimax-opt}.\qed
...
constant gap between the boxed leading constants is likewise genuine and not claimed tight
(Remark~\ref{rmk:opt-scope}(ii)), and it factors cleanly into two distinct sources:
\emph{(a)} the known looseness of Bretagnolle--Huber against the
exact Gaussian frontier $n_{\mathrm{opt}}=(\sigma/\Delta)^2 z_{1-\alpha}^2$, which the validator
confirms satisfies $n_{\mathrm{LB}}\le n_{\mathrm{opt}}$ with
$n_{\mathrm{opt}}/n_{\mathrm{LB}}\approx3.3$--$3.6$ throughout (so the lower bound never exceeds
the true frontier --- the bound is sound, not optimistic); and \emph{(b)} the certificate's
\emph{abstain-band penalty}, an additional factor $\approx4$ of $n_{\mathrm{opt}}$. Source (b) is
the direct cost of the symmetric abstain band: certifying \textsc{adapt} requires the whole
$(1-\alpha)$ confidence ball to clear $0$, i.e.\ $\varepsilon_n<\Delta/2$, which is a factor $2$
smaller radius --- hence a factor $4$ larger $n$ --- than merely placing the optimal test's
threshold at $0$ ($\varepsilon_n<\Delta$ would be the naive, and \emph{wrong}, requirement: at
$\varepsilon_n=\Delta$ the worst-case commit rate is $\Phi(0)=\tfrac12\not\le\alpha$). Multiplying,
the certificate-to-lower-bound ratio is $\approx3.4\times4\approx14$ at moderate $\alpha$, tending to
the clean $16$ as $\alpha\to0$; the validator confirms
$n_{\mathrm{LB}}\le n_{\mathrm{cert}}\le\tfrac{8\sigma^2}{\Delta^2}\log(2/\alpha)$, i.e.\ the
corrected constant is a true upper bound on the simulated certificate. We keep these two factors
distinct: only the $\to3.3$--$3.6$ belongs to the \emph{optimal test} ($n_{\mathrm{opt}}$); the
$\to16$ belongs to the \emph{certificate}.
\end{remark}

\begin{remark}[The constant gap, decomposed exactly (machine-checked)]\label{rmk:opt-decomp}
The certificate-to-lower-bound constant ($\to16$) factors into two parts that are now pinned down
exactly. \textbf{(a) The two-point lower bound is tight against the optimal test.} Replacing the
Bretagnolle--Huber surrogate by the \emph{exact} Gaussian two-point affinity
$1-\TV(P^n_+,P^n_-)=2\Phi(-\sqrt n\,\Delta/\sigma)$ and inverting $2\alpha\ge1-\TV$ gives the
optimal-test frontier $n_{\mathrm{opt}}=(\sigma/\Delta)^2 z_{1-\alpha}^2$ \emph{with equality}
(the threshold test $\sign(\bar X)$ attains both errors $=\alpha$ there). So the
$n_{\mathrm{opt}}/n_{\mathrm{LB}}$ gap is \emph{pure surrogate looseness}, not information loss;
the ratio $2z_{1-\alpha}^2/\log(1/4\alpha)$ stays near $3.3$--$3.7$ for every practical $\alpha$
and rises to $4$ only as $\alpha\to0$ (extremely slowly). \textbf{(b) The remaining factor is the
certificate's confidence-ball choice, not a universal abstention penalty.} In the two-world
problem a vanishing band ($\tau\to0$, the bare sign test) already certifies at $n_{\mathrm{opt}}$;
the certificate pays its extra $\approx4\times$ because it abstains over its full $(1-\alpha)$ ball
($\tau=\varepsilon_n$, forcing $\varepsilon_n<\Delta/2$). That $4\times$ becomes an \emph{exact
all-rules floor} $4\,n_{\mathrm{opt}}$ only once a third, on-frontier world ($\mu=0$, required to
abstain at level $\alpha$) is added (pairwise Le~Cam at half-separation $\Delta/2$); among
threshold rules the symmetric band then achieves $\kappa(\alpha)\,n_{\mathrm{opt}}$ with
$\kappa(\alpha)=(1+z_{1-\alpha/2}/z_{1-\alpha})^2\in[4,5.2]\downarrow4$, leaving a residual
$\kappa/4\in[1,1.30]$ open for arbitrary randomized rules. Both facts are machine-checked
(\texttt{theory\_v2/val\_tight\_constants.py}); the tight constant itself remains open.
\end{remark}

\subsection{Relation to the margin-rate view}

Theorem~\ref{thm:minimax-opt} is the \emph{decision-theoretic} counterpart of the margin-rate
view: bounding the smallest \emph{$(\alpha,\beta)$-reliable margin} $\kappa_n$ as a function of
$n$ and inverting $\kappa_n\asymp\sigma\sqrt{\log(1/\bar\alpha)/n}$ for a fixed margin $\Delta$
recovers exactly the sample-complexity statement here. The present formulation makes three things
explicit that the margin form leaves implicit: (i) the minimax object is the two-sided committal
error of a \emph{decision} rule on the identifiable side, not a margin; (ii) the $\log(1/\alpha)$
factor is a \emph{two-sided} (power-side) effect, distinguishing it cleanly from the one-sided
constant floor of Theorem~\ref{thm:imp} and Proposition~\ref{prop:lecam-finite}, which carry
\emph{no} logarithm; and (iii) the certificate \emph{already deployed} is the achiever, so
optimality is a property of the shipped rule, adaptively in $\sigma$. The lower bound here and the
margin-rate two-point estimate are the same Bretagnolle--Huber object viewed through margin vs.\
sample-size duality.
...
complexity $\tfrac{8\sigma^2}{\Delta^2}\log(2/\alpha)+O(R/\Delta\cdot\log(1/\alpha))$ matches the
information-theoretic floor $\tfrac{\sigma^2}{2\Delta^2}\log\tfrac1{4\alpha}$ over all
$\alpha$-valid label-free rules, up to a universal constant ($\to16$ on the leading term). No
...
\textbf{[B]} the exact optimal-test frontier $n_{\mathrm{opt}}=(\sigma/\Delta)^2 z_{1-\alpha}^2$
satisfies $n_{\mathrm{LB}}\le n_{\mathrm{opt}}\le n_{\mathrm{cert}}^{\mathrm{bound}}$ for every
$(\Delta,\alpha)$ (where $n_{\mathrm{cert}}^{\mathrm{bound}}=\tfrac{8\sigma^2}{\Delta^2}\log(2/\alpha)$),
has log--log slope $2.00$ in $1/\Delta$ (theory $2$), and $n_{\mathrm{opt}}/n_{\mathrm{LB}}$ is a
universal constant $\approx 3.3$--$3.5$ for $\alpha\le0.1$; the certificate-bound-to-lower-bound
leading-constant ratio $\to16$;
\textbf{[C]} the deployed empirical-Bernstein certificate keeps false-adapt $\le\alpha$ at
\emph{every} $n$ (validity), its full two-sided certifying-$n$ satisfies
$n_{\mathrm{LB}}\le n_{\mathrm{cert}}\le\tfrac{8\sigma^2}{\Delta^2}\log(2/\alpha)$ (the corrected
bound is a true upper bound on the simulated certificate), and its empirical certifying-$n$ grows
linearly in $\log(1/\alpha)$ at the upper-bound slope and as $1/\Delta^2$ (log--log slope $2.14$),
i.e.\ at the predicted rate;
...
Left: the optimal-test frontier $n_{\mathrm{opt}}=(\sigma/\Delta)^2 z_{1-\alpha}^2$, the
certificate bound $n_{\mathrm{cert}}^{\mathrm{bound}}=\tfrac{8\sigma^2}{\Delta^2}\log(2/\alpha)$,
and the two-point lower bound $n_{\mathrm{LB}}=\tfrac{\sigma^2}{2\Delta^2}\log\tfrac1{4\alpha}$
are parallel and linear in $\log(1/\alpha)$ (matched rate; the bounded vertical gaps are universal
constants: $n_{\mathrm{opt}}/n_{\mathrm{LB}}\to3.3$--$3.6$, and
$n_{\mathrm{cert}}^{\mathrm{bound}}/n_{\mathrm{LB}}\to16$). Right: all three lie on slope-$1$
log--log lines in $1/\Delta^2$, i.e.\ $n\asymp\sigma^2/\Delta^2$.}
