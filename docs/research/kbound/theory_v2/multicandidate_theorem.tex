...
the shared exchangeable calibration sample assumed above, and also when the $K$ adapters are scored
on a common deployment input so the estimates $\hat\Delta_k$ are coupled; the \v Sid\'ak branch
therefore applies only with disjoint per-candidate calibration draws \emph{and} independent
deployment estimates, while Bonferroni---valid under arbitrary dependence---is the assumption-light
