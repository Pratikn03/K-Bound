...
labels---under two complementary evidentiary regimes. K-Bound operates in the deductive regime of
a single unlabeled test stream, establishing the fundamental impossibility of auditing target risk
without an exogenous calibration sensitivity parameter $\beta$. KGA operates in the inductive regime
where historical benchmark shifts are available, fitting an empirical benefit predictor on past
evaluation cells and calibrating its residual uncertainty to govern a three-way commitment gate.
We do not claim KGA implements or approximates the population frontier; rather, K-Bound defines
what is fundamentally impossible from unlabeled test data alone, and KGA demonstrates what becomes
operational once historical shift benchmarks are admitted into the evidentiary basis.
...
equivalence). In practice, a decision-maker
evaluates whether a strict action $|M| > \beta$ remains stable across a domain-plausible range of
budgets $\beta$; the audit floor proves that no label-free audit can self-consistently estimate
$\beta$ away without external structural assumptions.
...
\centering
\scriptsize
\caption{Correspondence between population theory (K-Bound) and the empirical decision rule (KGA). A shared target functional does not make the quantities equal.}
...
\end{multline*}
K-Bound establishes what an entire declared family of target worlds permits: if every allowed drift in $\mathcal C_\beta$ preserves one strict direction ($|M|>\beta$), that direction is population sound; otherwise the maximal uniform rule must abstain. KGA addresses the realized deployment unit under a separately declared operational transfer model: it predicts the empirical risk difference $h_\theta(Z)$ and commits only when a calibrated interval excludes zero. The common decision problem is the paired deployment choice; the population theorem brackets the population benefit $\Delta$ through $(M,\gamma,\beta)$, whereas KGA brackets the empirical cell benefit $\Delta^{\mathrm{cell}}$ through $(\widehat\Delta,\varepsilon)$ at the cell level, and through $(\widehat\Delta, \varepsilon+b)$ at the population level via Proposition~\ref{thm:population-transfer} (\KBSuppRef{thm:app-pop-cert}{Theorem}{the supplementary population certificate theorem} in \KBSuppRef{app:transfer-proof}{Appendix}{the supplementary transfer proof}). Table~\ref{tab:theory-algorithm-bridge} makes this correspondence explicit.
...
\emph{Protocol A: Leave-one-cell-out empirical order-statistic calibration} (used in earlier historical controlled grids). Benefit predictions $\widehat\Delta_i$ were cross-fitted on the remaining $n-1$ cells, and each scored residual $r_i = |\widehat\Delta_i - \Delta_i^{\mathrm{cell}}|$ was excluded from the pool used to compute its own radius $\varepsilon_i$. However, this historical leave-one-out construction is peer-dependent: for $j \neq i$, the model $h_{-j}$ was fitted on training data containing $\Delta_i^{\mathrm{cell}}$, so the scored cell's outcome could indirectly influence other residuals $r_j$ in its calibration pool through model fitting.
...
$\CIFAReataAdapt$, and $\CIFAReataFreeze$. Their current actions are 1,094/334/732 and
1,207/118/835, respectively; both have zero observed false adaptations.
SAR makes 1,430/0/730 decisions and has regret $0.0017$, versus $0.0003$ for always-adapt.
...
Historical run-seed bootstrap vectors from earlier development cycles are cataloged separately in the supplement.
...
\KBSuppRef{app:auxiliary-results}{Appendix}{the supplementary auxiliary-results section} reports the valid diagnostic rows on separate metric scales, together with the
reasons why iWildCam, historical POEM/AETTA ports, and the focused Office-Home route cannot support current
...
data alone, so any population guarantee without coupled target labels requires treating $\beta$ as an
externally declared sensitivity parameter (by analogy to sensitivity analysis in causal inference), while
KGA pursues empirical interval coverage through historical shift benchmarks.
...
\begin{enumerate}
  ...
  (\KBSuppRef{app:historical-ports}{Appendix}{the supplementary historical ports section}, Table~\ref{tab:native-component-45}) evaluate fixed policies and
  ...
  constrained by documented method-specific dependencies: no authenticated outputs from Kim et al.'s TTA/ALine~\cite{kim2024ttaline} are available for our scored panel (0/15 available), Baek et al.'s ALine~\cite{baek2022aol} has partial component availability (11/15 available),
