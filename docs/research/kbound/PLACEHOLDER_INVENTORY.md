...
- *Name:* `tests/test_storage_manifest_coverage.py`
- *Problem:* `docs/research/kbound/STORAGE_MANIFEST.json` currently carries checksums for
  **3 files** (`claim_ledger.json`, `KBOUND_MIXED_STREAM_v2.json`,
  `KBOUND_WIN_BOOTSTRAP_CIS_oof.json`). Every other artifact behind a published table is
  unchecksummed, which is how `claim_ledger.json` was able to drift from its own recorded hash
  (F4-15) without any gate firing.
...
- *Regeneration:* `STORAGE_MANIFEST.json` must be regenerated by
  `scripts/reproduce_submission.sh` at freeze time so the recorded hashes are, by construction,
  the hashes of the released files.
