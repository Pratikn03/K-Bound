...
panel, with zero observed false adaptation. A separate Office-Home replication has a small point
edge over both fixed policies, but its seed interval includes zero. Every row needs a scope
qualification. Camelyon17's OOD subset contains no harmful cell; RxRx1 and the primary iWildCam and
Office-Home panels make zero \textsc{adapt} decisions; and the source-replayed Office-Home/iWildCam transfer rows
lack a predeclared uniform A7 full-fit-versus-LOO stability bound. On the remaining three natural tracks the outcome is different and we report it as such: PACS is
...
always-adapt ($0.0064$) on $8$ of $10$ backbones; and CIFAR-10.1 fails the declared transfer bar at
$\mathrm{FA}_{\mathrm u}=0.167$. Point-estimate no-harm is therefore a scoped description of the one-sided tracks, not a
...
$2{,}845$ strict decisions among $4{,}320$ cell--seed evaluations ($65.9\%$ decision coverage),
including $2{,}357$ \textsc{adapt} decisions with no observed false adaptations. The utility result
...
rate. Across the eligible Tent/EATA CIFAR-10-C arms the certificate makes $2{,}845/4{,}320$ strict
decisions at zero observed false adaptations, over $1{,}113$ and $1{,}244$ \textsc{adapt} decisions
(Clopper--Pearson $95\%$ upper bounds $0.0027$/$0.0024$ on the conditional rates). Tent's shipped
point beats both fixed policies with intervals excluding zero at every declared resampling unit;
EATA's adapt-side interval includes zero at the corruption unit. Under leave-one-corruption-out
refitting the radius inflates $4.3\times$--$6.9\times$, commitment falls from $0.51$--$0.60$ to
$0.40$--$0.42$, and the false-adapt count remains zero in all ten runs, but the adapt-side utility is
not CI-supported at the six-family unit. Where it buys nothing we say so: ImageNet-R is
$2.35\times$ worse than always-adapt, with per-backbone value per committed decision exactly zero on
$9$ of $10$ backbones.
\end{enumerate}
...
Harmful-dominated & RxRx1; iWildCam/Office-Home & tie always-freeze, beat always-adapt & primary panels: descriptive no-harm with $0$/$0$/$0$ adapts; Office-Home replication: small non-robust point edge; A7 open \\
Domain-gen.\ natural & PACS (4 LODO splits) & no-harm or abstain & \textbf{null; loses to always-adapt} $2.45\times$ \\
Mixed + detectable & CIFAR-10-C Tent/EATA & adapt helpful, freeze harmful, abstain near zero & locked; $0$ false adapts in $2357$; CI beats-both for Tent, point beats-both for EATA \\
...
\midrule
CIFAR-10-C stress & corruption, composition, and update stress & Tent/EATA (SAR withheld) & mixed detectable decision benchmark \\
...
The results are organized by regime rather than by dataset prestige, and they are read as a safety evaluation. The point estimate on each of the four one-sided natural tracks is no-harm, with the exposure and transfer-validity qualifications reported in Table~\ref{tab:uniform-panel}; on the three remaining natural tracks KGA is a conservative null or a declared failure, which we report in the same table rather than in a footnote. Controlled mixed regimes then test action discrimination and selective coverage and supply the secondary beats-both mechanism confirmation, and low-margin transfers test whether the method declines unsupported claims. Accuracy and regret are reported as descriptive task metrics throughout, not as the controller's objective.
...
\begin{tabular}{@{}p{0.14\textwidth}p{0.13\textwidth}cp{0.26\textwidth}p{0.15\textwidth}p{0.15\textwidth}@{}}
\toprule
...
iWildCam H v2 & seed-0 calibration; seed-1 test $n=72$ & 0$^{\dagger}$ & $0.0041/0.1028/0.0041$; $\mathrm{FA}_{\mathrm u}=0$ & ties freeze; no \textsc{adapt}, one test seed, A7 open & \textbf{source-record replay}: original hash plus compact records released (App.~\ref{app:claim-artifact}) \\
Office-Home M v2 & target test $n=35$, seeds $0,1$ & 0$^{\dagger}$ & $0.0158/0.0468/0.0158$; $\mathrm{FA}_{\mathrm u}=0$ & ties freeze; no \textsc{adapt}; A7 open & \textbf{source-record replay}: descriptive no-harm \\
...
ImageNet-R D & $4$ seeds $\times10$ backbones $\times12$ conditions & 165 & aggregate regret $0.0150/0.0064/0.0325$; $0/480$ false adapts; per-backbone breakdown in Table~\ref{tab:imagenetr-backbone} & \textbf{worse than always-adapt on $8$ of $10$ backbones}; $4$ backbones have a $0\%$ harmful base rate & source-replayed exact-LOO diagnostic null \\
...
Table~\ref{tab:multiseed} cells and the aggregate-only PACS summaries) the ceiling is $0$ and the
statistic carries no information. ImageNet-R is no longer in that category: its promoted row is the
leave-one-condition-out replay, for which a nonzero miss is possible.
...
to $1/135$. The source-replayed ImageNet-R zero is likewise measured under leave-one-out calibration;
the in-pool $n\le18$ zeros remain arithmetic.
...
Office-Home and iWildCam are undefined because there is no \textsc{adapt}.
Four tracks (RxRx1, Office-Home, and iWildCam with $0$ adapts, plus the D33 mechanism check with $9$) make fewer than $10$ \textsc{adapt} decisions and
...
The conclusion we draw is deliberately narrow: \textbf{the certificate has substantial
false-adapt exposure on CIFAR-10-C and ImageNet-R; routing utility is CI-supported only on the
CIFAR-10-C Tent stress track}.
...
\textsc{adapt} decisions: guarantee untested. CIFAR-10-C and ImageNet-R have nontrivial exposure;
only CIFAR-10-C Tent also has CI-supported routing utility.}
...
Office-Home M v2$^{\dagger}$ & 35 & 0 & 11 & 24 & 0 & $0.0000$ & undef. \\
PACS (pooled) & 216 & 12 & 164 & 40 & 2 & $0.0093$ & $0.4381$ \\
iWildCam H v2$^{\dagger}$ & 72 & 0 & 24 & 48 & 0 & $0.0000$ & undef. \\
...
\textsc{adapt} could have been false. Office-Home, iWildCam, ImageNet-R, CIFAR-10-C, ImageNet-C,
PACS and Camelyon17 counts are now source-record or per-cell recomputations; RxRx1 and CIFAR-10.1
retain their locked aggregate accounting.}
\end{table}
...
the certificate does commit rarely (Office-Home M-v2: $0$ \textsc{adapt}, $11$ \textsc{freeze},
$24$ \textsc{abstain}; RxRx1 and iWildCam also make $0$ \textsc{adapt}). This subsection answers the objection with numbers, and
...
$\kappa=0.9$ (regret $0.00134$ at yield $0.684$). On ImageNet-C, moving from $\kappa=1$ to $0.5$
...
$\kappa{=}1$ is the canonical certificate; ImageNet-C and ImageNet-R use the source-replayed
exact-LOO radius).
...
the heuristic's favour. Source: \texttt{decision\_value\_results.json}, synchronized from the
canonical reconciled panel for ImageNet-C and ImageNet-R.}
...
$\kappa=0.5$ & $0.763$ & $0.00074$ & $0.558$ & $0.00944$ & $0.769$ & $0.00542$ \\
$\kappa=0.9$ & $0.684$ & $\mathbf{0.00134}$ & $0.380$ & $0.01408$ & $0.469$ & $0.01305$ \\
\textbf{$\kappa=1$ (canonical)} & $\mathbf{0.668}$ & $\mathbf{0.00150}$ & $\mathbf{0.368}$ &
$\mathbf{0.01468}$ & $\mathbf{0.404}$ & $\mathbf{0.01497}$ \\
$\kappa=2$ & $0.546$ & $0.00312$ & $0.272$ & $0.01673$ & $0.031$ & $0.03073$ \\
\midrule
...
beat the source-replayed certificate ($0.00636$ vs.\ $0.01497$) while adapting on every
cell at a harmful-cell rate of $0.202$, against the certificate's $0/480$ false adaptations.
...
cells, not at random. Under the corrected exact-LOO radius ImageNet-C Tent makes no commitment and
ties freeze; the single-commitment permutation result from the superseded radius is therefore not
reused. ImageNet-R is the counter-case: its exact-LOO replay makes $194$ decisions but still loses to
always-adapt by $2.35\times$. The previous per-commitment permutation artifact used the superseded
operating point; \textbf{DRAFT TODO: regenerate that secondary permutation diagnostic before making
a per-commitment-value claim}. This missing secondary diagnostic does not affect the aggregate
regret, action counts, or false-adapt result.
...
radius probe. Under the corrected radius the baseline yield is $0\%$; \textbf{DRAFT TODO: rerun the
counterfactual power probe before quoting its target yield}. The qualitative diagnosis---estimator
...
(Table~\ref{tab:decisive}); CIFAR-10-C SAR is withheld. The corrected ImageNet-C exact-LOO panel is
...
guarantee. Office-Home and iWildCam are now replayed from released, source-hashed compact records
(App.~\ref{app:claim-artifact}); both make zero \textsc{adapt} decisions and tie freeze. They remain descriptive because
the predeclared uniform A7 stability premise is absent. PACS and ImageNet-R are completed null
...
runs. Elsewhere it is point-estimate no-harm on the four one-sided natural tracks --- each with the
qualification recorded in \S\ref{sec:limits} --- and where it buys nothing we say so: PACS,
