...
if __package__:
    from . import authority, schema, paths
else:  # pragma: no cover
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
    from kbound_repro import authority, schema, paths


def _discover_promoted_tex(root: Path) -> list[str]:
    pats = [
        "docs/research/kbound/**/kbound_short*.tex",
        "docs/research/kbound/**/main*.tex",
    ]
    found: list[str] = []
    for pat in pats:
        found += glob.glob(str(root / pat), recursive=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for t in tex:
        try:
            manuscript_texts[str(Path(t).relative_to(root))] = Path(t).read_text(errors="ignore")
        except (OSError, ValueError):
            try:
                manuscript_texts[t] = Path(t).read_text(errors="ignore")
            except OSError:
                pass
