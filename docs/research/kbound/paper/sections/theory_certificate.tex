...
The quantities have different definitions. The declared bound $\beta$ limits the absolute target disagreement-conditional residual, whereas $\varepsilon$ is calibrated from prediction errors for the empirical target. They may coincide numerically, but that does not make them interchangeable.
\end{remark}

\begin{proposition}[Cell-to-population transfer via compound bound]\label{thm:population-transfer}
Let $m\ge1$ be an integer and let $\Delta^{\mathrm{cell}}=\frac{1}{m}\sum_{j=1}^m W_j$ be the measured cell benefit on $m$ fresh,
...
An alternative route directly operationalizes the population frontier $|M|>\beta$. Fix the
predictor pair and score independently of a fresh i.i.d.\ sample of unlabeled target inputs,
and let $n_D>0$ of those inputs fall in $D$. The empirical disagreement margin
