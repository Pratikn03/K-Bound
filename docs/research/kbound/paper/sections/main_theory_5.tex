...
% The 2026-08-31 review identified missing/overstrong H and rank-one conditions.
% See audits/THEOREM_NOVELTY_BIBLIOGRAPHY_REVIEW.md, Section 10. This retained
% historical source is not evidence of full foundational Lean closure.
% ============================================================================
...
\begin{align*}
R(\hat g)-R(g^\star)
  &=\E\bigl[|\Delta(Z)|\,\mathbf 1\{\sign\hat\Delta\neq\sign\Delta\}\bigr],\\
g^\star&=\mathbf 1[\Delta>0].
\end{align*}
...
\begin{align*}
&\inf_{\hat g}\ \max_{\theta=\pm1}
  \bigl[R_{P_\theta}(\hat g)-R_{P_\theta}(g^\star)\bigr]\\
&\quad\ge \tfrac{\Lambda}{2}\Bigl(1-\TV\bigl(Q^{(n)}_{-1},Q^{(n)}_{+1}\bigr)\Bigr)\\
&\quad\ge \tfrac{\Lambda}{4}\,e^{-\mathrm{KL}(Q^{(n)}_{-1}\|Q^{(n)}_{+1})}.
\end{align*}
...
minimax committal error is $0$; if $|M|\le\beta$ the minimax error is $\tfrac12$.
...
testing plus flip-minimization.
\emph{Proof status.} This is a sketch and no full proof of this theorem appears in this
manuscript; we mark it as such rather than let a reader assume one is available. No claim in
the impossibility spine of \S\ref{app:beta-impossible}--\S\ref{app:episode-beta}, and no claim
in the experimental sections, depends on it. The repository files
\texttt{theory\_validation/val\_conj1\_*.py} are \emph{numerical validations} of the stated
identities on synthetic instances; a validator is not a proof, and they are cited here as
evidence that the statement is not mis-transcribed, not as evidence that it is proved.
\end{proof}
...
\emph{Proof status.} As for Theorem~\ref{thm:conj1-dichotomy}: a sketch, with no full proof in
this manuscript, and \texttt{val\_ev\_rate.py} is a numerical validation rather than a proof.
Nothing in the impossibility spine or the experiments depends on it.
\end{proof}
