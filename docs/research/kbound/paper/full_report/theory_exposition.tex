...
\end{equation}
and compare it on $D$ with a label-free proxy $u(x)$ that is integrable under
$P_X(\cdot\mid D)$.  Defining a proxy mean and a residual yields an
...
For a general loss with both model losses integrable under the target joint law,
define the random loss advantage
\begin{equation}
...
\end{equation}
For a label-free proxy $t(X)$, assume $\E[|G(X,Y)|]<\infty$ and
$\E[|t(X)|]<\infty$ under the same target joint law.  These integrability
conditions justify the identity
\begin{equation}
  \Delta=\E[t(X)]+\E[G(X,Y)-t(X)].
  \label{fr:eq-general-loss-decomposition}
\end{equation}
This decomposition suggests a margin-plus-residual analysis.  A declared absolute bound on the second term is sufficient
