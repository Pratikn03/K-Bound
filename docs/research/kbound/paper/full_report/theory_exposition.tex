...
analysis would either condition on the realized augmented object, enlarge the class to cover the
estimation error, or prove a separate sampling statement.  None of those steps is obtained merely
by reusing the symbol $M$ for a sample average.
...
absolute value below $\beta$.  They preserve all label-free evidence and yield benefits $2dr$ and
$-2dr$.  The ambiguity is exact: no additional sample from the same label-free evidence law can
...
also use an independent random seed $U$ and return
\begin{equation}
...
$X$ nor either model's output at a given $X$.  Consequently, it preserves every label-free target
statistic formed from inputs and the fixed model pair.  It also preserves any source or development
sample whose law has been fixed independently of the deployment label kernel.  At the same time,
...
label kernel on $D$, hold the law of $L_S$ fixed while applying the target kernel construction.
Both worlds then induce the same joint law of $(W_0,L_S)$ and can still have different values of
