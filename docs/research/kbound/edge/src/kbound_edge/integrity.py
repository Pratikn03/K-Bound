...
def check_feature_schema_unchanged(cfg: dict) -> tuple[bool, str, str, str]:
    ...
    expected = sorted(EDGE_EVIDENCE_NAMES)
...
def check_epsilon_conformal_split(results_dir: str) -> tuple[bool, str, str, str]:
    ...
    return (
        passed,
        f"fit={expected_fit}, conformal={expected_conformal}",
        f"fit={fit_sessions}, conformal={conformal_sessions}",
        summary_path,
    )
...
def check_heldout_excluded_from_calibration(results_dir: str) -> tuple[bool, str, str, str]:
    ...
    return passed, "0 overlapping sessions", f"{len(overlap)} overlaps: {sorted(overlap)}", summary_path
...
def check_identical_heldout_stream(results_dir: str) -> tuple[bool, str, str, str]:
    ...
    return (
        passed,
        f"all policies processed {n_windows_list[0]} windows",
        f"observed window counts: {n_windows_list}",
        metrics_path,
    )
