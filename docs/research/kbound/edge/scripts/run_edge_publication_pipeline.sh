...
echo "=== [12] Fail-closed publication gate ==="
"${PY}" 13_check_publication_gate.py --config "${CFG}" --strict

echo "=== [13] Dashboard snapshot ==="
bash "${ROOT}/docs/research/kbound/scripts/build_dashboard.sh"

echo ""
echo "=== DONE ==="
echo "Check: experiments/kbound/results/edge_real_phone_v1/model_card.json"
echo "       experiments/kbound/results/edge_real_phone_v1/heldout_metrics.json"
echo "       experiments/kbound/results/edge_real_phone_v1/camera_tables_values.tex"
echo "       experiments/kbound/results/edge_real_phone_v1/publication_gate.json"
echo "Dashboard study_status flips to 'verified' only when publication_gate.json passes."
echo "Maintained sources: docs/research/kbound/kbound_submission.tex and kbound_tmlr.tex"
echo "After evidence review: bash docs/research/kbound/scripts/build_pdfs.sh"
