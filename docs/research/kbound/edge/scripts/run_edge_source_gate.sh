...
METRICS="$ROOT/docs/experiments/kbound/results/edge_real_phone_v1/source_val_metrics.json"
if [ ! -f "$METRICS" ]; then
  # fallback: scan for latest metrics export
  METRICS="$(find "$ROOT/docs/experiments/kbound/results/edge_real_phone_v1" -name '*val*metrics*.json' 2>/dev/null | head -1 || true)"
fi
...
paths = [
    "$METRICS",
    os.path.join("$ROOT", "docs/experiments/kbound/results/edge_real_phone_v1/heldout_metrics.json"),
]
for p in paths:
