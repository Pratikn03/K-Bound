...
def main():
    ...
    for w in WINS:
        ...
        eps0 = m0["eps_global"]  # dev-fixed out-of-fold radius; frozen across resamples (the dev
        #                          calibration that fixes B_hat and eps is held constant by design)
        sk_kga, sk_ad, sk_fr = stored_regrets(w)
        n = len(test)
        df = np.empty(a.B); da = np.empty(a.B); fa = np.empty(a.B)
        for b in range(a.B):
            tb = [test[i] for i in rng.integers(0, n, n)]
            mb = sk.score_transfer(cal, tb, "gbr", "frozen", frozen_eps=eps0)
