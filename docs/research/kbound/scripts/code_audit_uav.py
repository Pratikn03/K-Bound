#!/usr/bin/env python3
"""Automated code audit for /Volumes/T9/uav (canonical: AutoML_Flagship_V8).
...
UAV = Path("/Volumes/T9/uav")
AUTO = UAV / "AutoML_Flagship_V8"
...
def count_loc() -> dict:
    stats = {}
    for label, root in [("AutoML_Flagship_V8", AUTO), ("kbound_only", KONLY), ("uav_root_src", UAV / "src")]:
        files = _iter_py(root)
...
def stale_root() -> list[str]:
    ...
    for d in ["src", "tests", "docs", "research_lock"]:
        ...
        if p.exists() and not any(p.rglob("*.py")) and not any(p.rglob("*.md")):
            notes.append(f"/Volumes/T9/uav/{d}/ is empty placeholder — use AutoML_Flagship_V8/")
    if (UAV / "kbound.log").exists():
        notes.append("Root kbound.log/aux are stale LaTeX artifacts (Jun 15); canonical papers in AutoML_Flagship_V8/docs/research/kbound/")
    return notes
...
def build_report() -> dict:
    ...
    return {
        ...
        "scope": "Automated pass over /Volumes/T9/uav (canonical AutoML_Flagship_V8)",
