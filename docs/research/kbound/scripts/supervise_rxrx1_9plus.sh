...
# BEFORE writing that seed's verified completion receipt. A filename alone is
# not completion evidence; the runner verifies its full configuration and hashes.
# An incomplete-but-exit-0 pass is also retried. The runner skips completed cells and
...
echo $$ > "$LOCK"
trap 'rm -f "$LOCK"' EXIT
if [ -f "$ALLDONE_PATH" ]; then
  mv "$ALLDONE_PATH" "$ALLDONE_PATH.stale.$$"
fi
...
discard_stale_receipt() {
  [ -f "$DONE" ] || return 0
  mv "$DONE" "$DONE.stale.$$.$i"
  echo "[sup9] seed $S preserved invalid/stale completion receipt $(date)" >> "$SUPLOG"
}
...
for S in "${MODEL_SEEDS[@]}"; do
  ...
  while [ $i -lt $MAX ]; do
    ...
    if completion_receipt_valid; then
      echo "[sup9] seed $S verified receipt before attempt $i; next seed $(date)" >> "$SUPLOG"; break
    fi
    discard_stale_receipt
    ...
    if completion_receipt_valid; then
      echo "[sup9] seed $S verified receipt after attempt $i; next seed $(date)" >> "$SUPLOG"; break
    fi
    discard_stale_receipt
...
if [ $ALLDONE -eq 1 ]; then
  echo "$(date) all model seeds verified: ${MODEL_SEEDS[*]}" > "$ALLDONE_PATH"
fi
