...
def load_candidate(source_dir: Path, candidate: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for path in files:
        ...
        current_epsilon = current.radius
        current_decisions = current.action
        ...
        for index, row in enumerate(records):
            ...
            replayed = dict(row)
            replayed["current_policy_prediction"] = float(current_prediction[index])
    ...
    *,
    ...
    return {
        ...
        "current_policy_replay": {
            ...
            "radius": "exact split-conformal rank radius on a score-fold-disjoint calibration subset",
            "historical_fields_used_for_scoring": False,
