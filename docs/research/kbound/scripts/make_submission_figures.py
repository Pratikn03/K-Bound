...
def frontier():
    # M is a disagreement-conditional correctness margin and therefore lives
    # in [-0.5, 0.5].  Keep the schematic budget inside that feasible range;
    # a value such as b=1.0 would depict an impossible frontier.
    b, limit = 0.1, 0.5
    fig, ax = plt.subplots(figsize=(6.4, 2.5))
    ax.axvspan(-b,b,color="#f0a000",alpha=0.25); ax.axvspan(b,limit,color=GREEN,alpha=0.18); ax.axvspan(-limit,-b,color="#d6604d",alpha=0.18)
    for x in (-b,b): ax.axvline(x,color="k",lw=1.2)
    ax.axvline(0,color="k",lw=0.6,ls=":")
    ax.text(0.30,0.5,"ADAPT\n(knowably helpful)",ha="center",va="center",fontsize=9,color=GREEN)
    ...
    ax.text(0,-0.43,r"Abstention-band width  $2\beta$",ha="center",va="top",fontsize=8.5)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Regenerate K-Bound submission figures")
    parser.add_argument(
        "--frontier-only",
        action="store_true",
        help="regenerate only the conceptual population-frontier figure",
    )
