...
FINAL_BUILD_LOG = KBOUND / "kbound_short_final_build.log"
...
def paper_page_count() -> int | None:
    if not FINAL_BUILD_LOG.is_file():
        return None
    match = re.search(r"Output written on .*?\((\d+) pages?", FINAL_BUILD_LOG.read_text(errors="replace"))
...
def build_snapshot() -> dict[str, Any]:
    ...
    return {
        "meta": {
            "build_id": f"manifest-{manifest_digest[:12]}",
