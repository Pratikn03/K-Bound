...
def per_condition(cal, test):
    ...
    # out-of-fold (leave-one-out) calibration residuals -> valid conformal radius (no in-sample
    # leakage); the in-sample radius was ~10x too small on these small dev sets.
...
DATASETS = [
    dict(name="Camelyon17", kind="single",
         ...
         domains=["test", "val"]),  # OOD only; exclude in-distribution id_val (Phase-1 audit)
...
def load(ds):
    if ds["kind"] == "transfer":
        ...
        if ds.get("domains"):
            raw = json.load(open(str(ROOT / ds["rec"])))["records"]
            doms = set(ds["domains"])
            recs = [af._one_record(r, candidate=ds["cand"]) for r in raw
                    if r.get("candidate") == ds["cand"] and r.get("domain") in doms]
        else:
