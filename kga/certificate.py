...
:func:`kga.policy.decide` yields the ADAPT/FREEZE/ABSTAIN trichotomy. Theorem 3
controls marginal false adaptation when the interval coverage premise holds.
...
* :func:`evalue_anytime` -- an experimental testing-by-betting interface whose
  anytime interpretation requires the stated process assumptions.

All estimators are pure ``numpy``/``math``, deterministic, and torch-free.

The package is self-contained and does not import an adapter implementation.
"""
...
# ---------------------------------------------------------------------------
# Standard Maurer-Pontil empirical-Bernstein lower confidence bound.
...
# ---------------------------------------------------------------------------
# (4) Experimental e-value recursion (Ville / testing by betting)
# ---------------------------------------------------------------------------
def evalue_anytime(
    paired_benefits: np.ndarray,
    *,
    ...
    """Experimental e-value certificate based on testing by betting.
    ...
    Under the required predictability, boundedness, and supermartingale
    assumptions, Ville's inequality gives simultaneous type-I control. This
    function implements the recursion; callers remain responsible for verifying
    those assumptions in their data-generating process.
