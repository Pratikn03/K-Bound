"""Controlled CIFAR-C and ImageNet-C adaptation stress-grid runner.

The grid varies corruption, severity, stream composition, batch regime, and
adapter aggressiveness. Adaptation and evaluation samples are separated. KGA
uses label-free evidence for each decision; labels are retained only for
offline benefit, regret, and false-adapt scoring.

The per-run radius is an exact observed order statistic over cross-fitted
condition residuals. This is empirical residual calibration, not an independent
split-conformal guarantee. Tent, EATA, and SAR are protocol-matched local
implementations rather than claims of official upstream execution.
"""
...
def decide_kga(Z, B, alpha=ALPHA, n_estimators=250, max_depth=2, lr=0.05, seed=SEED):
    """LOO benefit estimates plus an exact-rank empirical residual radius.

    The stress-grid residuals are cross-fitted evaluation-cell residuals, not an
    independent split-conformal sample. The saved provenance states that weaker
    empirical-calibration interpretation explicitly.
    """
        ...
        for mth, rws in rows.items():
            ...
            for i, r in enumerate(rws):
                ...
                _per_cond.append({
                    ...
                    "eps_conformal": eps_i,                # exact-rank empirical residual radius
            ...
            with open(_pc_path, "w") as _f:
                json.dump({"seed": SEED, "benchmark": bench, "method": mth,
                           ...
                           "per_condition_fields_absent_reason":
                               "v2 KGA = gradient-boosted B_hat(Z) + an exact-rank empirical eps; "
