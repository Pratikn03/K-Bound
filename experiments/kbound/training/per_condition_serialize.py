...
INTEGRITY
  ...
  - decide_benefit() prefers the REAL certificate analysis.decide_kga (sklearn
    GradientBoostingRegressor + exact-rank empirical eps). If sklearn is unavailable in the
    runtime (e.g. a torch-free CPU verification sandbox), it falls back to a clearly
    labelled numpy k-NN-in-Z benefit estimator with the SAME exact-rank empirical radius and
...
def _decide_kga_numpy(Z, B, alpha, k=8):
    ...
    exact-rank empirical radius and the SAME decision rule as the production certificate.
