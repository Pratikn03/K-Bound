...
def test_design_status_or_decision_doc_exists():
    if not (DESIGN.exists() or DECISION.exists()):
        pytest.fail("at least one of FAMILY_D_V2_DESIGN_STATUS.md or FAMILY_D_V2_DATASET_AND_PROTOCOL_DECISION.md must exist")
...
def test_no_v2_decision_doc_lists_visa_as_candidate():
    ...
    if "VisA" in t:
        ...
        assert any(s in ctx for s in ("ineligible", "exclude", "not be used", "not a v2", "removed")), (
