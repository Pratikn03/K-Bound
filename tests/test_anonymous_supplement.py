...
def _write_seal(
    ...
    *,
    ...
    if source_paths and root is None:
        raise ValueError("fixture source paths require a fixture root")
...
def _complete_release_controls(
    ...
    *,
    ...
    _write_seal(seal, marker, root=root, source_paths=extra_paths)
...
def test_anonymous_package_rejects_extra_protected_checksum_canary_before_io() -> None:
    extra = "experiments/kbound/so2sat/target_results/protected-canary.json"
    payload = "".join(f"{'0' * 64}  {path}\n" for path in (*REQUIRED_RELEASE_PATHS, extra)).encode("utf-8")

    with pytest.raises(PrivacyError, match="unexpected checksum entries"):
        supplement._parse_checksum_payload(payload)
...
def test_standalone_anonymous_verifier_rejects_forged_embedded_source_seal(
    ...
    forged["sealed_artifact_count"] = int(forged["sealed_artifact_count"]) + 1
