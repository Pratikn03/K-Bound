...
def test_selection_never_used_test_metrics(rows):
    for r in rows:
        ...
        assert flag in {"false", "0"}, f"selection_used_test_metrics must be False (Phase 1.C); row: {r}"
...
def test_selected_comparator_drawn_from_pool(rows):
    ...
    for r in rows:
        ...
        assert sel in pool, f"selected_comparator must be drawn from the predefined baseline pool; got {sel!r}"
