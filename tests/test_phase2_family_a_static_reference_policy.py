...
def test_analysis_driver_hard_codes_static_attention_as_only_comparator():
    ...
    for c in forbidden_comparator_iter:
        ...
        assert c not in src, (
            f"Family-A analysis driver references {c!r}; primary surface allows only static_attention"
        )
...
def test_analysis_driver_uses_k_equals_5_holm():
    ...
    assert ("K=5" in src and "holm_bonferroni" in src) or "holm_bonferroni" in src.replace(
        "K=5", ""
    )
...
def test_analysis_driver_does_not_overwrite_historical_pilot_paths():
    src = _source()
    assert "family_a_powered_ensemble_inference.csv" not in src.replace(
    ...
    assert "family_a_powered_holm_results.csv" not in src.replace(
