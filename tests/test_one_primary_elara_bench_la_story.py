...
def test_paper_uses_one_b1_b2_pair_in_abstract():
    ...
    assert (
        "+0.0506" in abstract and "+0.0319" in abstract
    ...
    assert (
        "+0.0367" not in abstract and "+0.0538" not in abstract
...
def test_thesis_uses_one_b1_b2_pair_in_abstract():
    t = THESIS.read_text()
    abstract = t[:5000]
    assert "+0.0506" in abstract and "+0.0319" in abstract, "thesis abstract missing PRIMARY B1/B2 pair"
    assert "+0.0367" not in abstract and "+0.0538" not in abstract, "thesis abstract uses SECONDARY deltas"
