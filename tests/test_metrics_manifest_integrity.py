...
def test_family_a_confirmatory_cells_have_required_fields(manifest):
    for c in manifest["claims"]:
        if c["analysis_status"] != "audited primary reanalysis":
            continue
        for f in ("rga_plus_head", "rga_plus_test_roc_auc", "rga_plus_validation_roc_auc",
                  "primary_comparator", "primary_comparator_test_roc_auc",
                  "primary_comparator_validation_roc_auc",
                  "delong_p_raw_single_rep_seed", "delong_p_holm_family_A_K5"):
...
def test_no_claim_uses_confirmatory_language(manifest):
    ...
    for c in manifest["claims"]:
        ...
        assert status in {"protocol-diagnostic", "audited primary reanalysis", "exploratory"}, (
...
def test_selection_rules_recorded(manifest):
    for c in manifest["claims"]:
        if c["analysis_status"] == "audited primary reanalysis":
            assert "validation_frozen" in (c.get("rga_plus_selection_rule") or ""), (
                f"cell {c['cell_id']} rga_plus_selection_rule must contain 'validation_frozen'"
            )
            assert "validation_frozen" in (c.get("primary_comparator_selection_rule") or ""), (
...
def test_source_artifacts_present(manifest):
    for c in manifest["claims"]:
        ...
        for k in ("rga_plus_selection_csv", "comparator_selection_csv",
                  "audited_inference_csv", "descriptive_variability_csv"):
            assert k in s and s[k], f"cell {c['cell_id']} missing source_artifact {k}"


def test_pairing_strength_recorded(manifest):
    valid = {"independent_modalities", "naturally_structured_views",
             "derived_view_proxy", "label_aligned_stress_only", "local_replay"}
    for c in manifest["claims"]:
...
def test_macros_contain_no_underscore_in_macro_name():
    ...
    for m in re.finditer(r"\\newcommand\{\\([^}]*)\}", text):
        name = m.group(1)
        assert "_" not in name and not any(ch.isdigit() for ch in name), (
