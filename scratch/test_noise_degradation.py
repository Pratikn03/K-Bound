...
# Import the synthetic training logic we developed
from scratch.test_independent_synthetic_anomaly_training import augment_with_synthetic_anomalies_independent
...
def test_noise_degradation():
    ...
    # Test set evaluation under noise injection degradation
    ...
    for cell_id, target in [("D-EYE-1 (NOISY)", "depth"), ("D-EYE-2 (NOISY)", "rgb")]:
        print(f"\nEvaluating {cell_id} ({target} noise corruption)...")
        # Apply noise degradation: replace target domain score with random uniform noise
        deg_feat = test_feat.copy()
        deg_mask = test_mask.copy()
        d_idx = list(domain_order).index(target)
        
        # Inject high-variance random noise into the score column of target domain
        deg_feat[:, d_idx, score_idx] = rng.uniform(0.0, 1.0, size=len(test_feat))
