...
Theorem~\ref{thm:disagree} and Proposition~\ref{thm:disagree-mc} this ordering is exactly what fixes
$\sign\Delta$. The price becomes a \emph{checkable} condition rather than an untestable
premise.

\begin{definition}[Conditional error-independence on $D$, with per-class symmetry]\label{def:cei}
On the disagreement region $D$ with latent label $Y$: (i) candidates' correctness indicators
$\{\mathbf1[f_a^{(j)}(X){=}Y]\}_{j=1}^M$ are \emph{conditionally independent given $Y$};
\emph{and} (ii) each candidate is \emph{per-class symmetric},
$\Pr(f_a^{(j)}{=}Y\mid Y{=}1)=\Pr(f_a^{(j)}{=}Y\mid Y{=}0)$.
Clause (ii) is load-bearing, not cosmetic: under (i) alone the agreement law carries the
exact deficit $c_{ij}-b_ib_j=\pi(1-\pi)\,\delta_i\delta_j$ ($\pi=\Pr(Y{=}1\mid D)$,
$\delta_j$ the per-class accuracy gap), so the rank-one identity below holds for a
$M\ge3$ panel iff every $\delta_j=0$ --- see Theorem~\ref{thm:H-deficit}, where (i)+(ii)
is named \textbf{H} and this insufficiency is proved and validated.
\end{definition}

\begin{proposition}[Multi-candidate ordering identifiability]\label{thm:multicand}
...
Combined with Theorem~\ref{thm:disagree} and Proposition~\ref{thm:disagree-mc}, $\sign\Delta$ for any
...
It is, however, \emph{not complete}: there exist strictly interior laws with $\tau=0$
that violate Definition~\ref{def:cei} and bias the recovered $b$ --- the constructive
witness and the exact characterization of what $\tau=0$ pins down (rank-one $c$, i.e.\
pairwise H-realizability) are given in Theorem~\ref{thm:no-verify}(b).
\end{proposition}
\begin{proof}
...
\includegraphics[width=0.92\linewidth]{figures/final/fig_feasibility.png}
\caption{Multi-candidate identifiability (Proposition~\ref{thm:multicand},
...
\paragraph{Weight (honest).} The estimator itself is \emph{not} ours: under conditional
error-independence the identity $2A_{ij}-1=b_ib_j$, the triple-product recovery
$b_i^2=c_{ik}c_{il}/c_{kl}$, and the better-than-chance sign anchor appear, equation for
equation, in \textbf{Platanios et al.}\ (UAI 2014), and the same rank-one structure powers
the spectral ranking of \textbf{Parisi et al.}\ (PNAS 2014) and the per-classifier
accuracy estimation of \textbf{Jaffe et al.}\ (AISTATS 2015). Platanios et al.\ also
already observe the $M\ge4$ overdetermination and use a residual to quantify dependence,
anticipating much of Proposition~\ref{prop:cei-test}; our residual adds only the explicit
closed-form vanishing-minor statistic and its sound one-sided reading. What this
subsection contributes is therefore \emph{not} the estimator but its \emph{decision use}:
wiring the classical agreement machinery into a label-free \emph{adapt/freeze certificate}
for $\sign\Delta$ on the disagreement region, with the diagnostic as a checkable guard on
the identifying assumption. So scoped, it \emph{enlarges} the knowable region beyond the
...
\emph{testable} --- the guarantee that agreement-based heuristics (ATC, AoL, AETTA) use
empirically but never establish. It is not an assumption-free result: it substitutes
conditional error-independence (often violated by co-trained models, which is exactly why
the diagnostic matters) for the earlier crutches, and its sign anchor is as untestable as
theirs. The assumption-free dichotomy --- the weakest checkable condition with a tight
matching converse --- remains open and is the stated frontier of this program
