...
def run_one_seed_window(cfg, seed, window_size, eid, benchmark, protocol):
    ...
    clean_fired = estimator.gate_decisions(clean_weights, test_mask, gate_mode="mean",
                                           gate_threshold=0.66)
    ...
    static_auc_clean = _safe_auc(test_labels, static_probs_clean)

    # RGA AUC on clean data
    rga_probs_clean, _ = _predict_craf_with_stats(
        model, estimator, test_feat, test_mask, device,
        clean_gate_threshold=0.66, per_sample_gating=True,
    )
    rga_auc_clean = _safe_auc(test_labels, rga_probs_clean)
    ...
    for deg_type in DEGRADATION_TYPES:
        ...
        deg_fired = estimator.gate_decisions(deg_weights, deg_mask, gate_mode="mean",
                                             gate_threshold=0.66)
...
def main() -> int:
    ...
    deg_fields = [
        "degradation_type", "reference_type", "detection_power", "gate_activation_rate",
        "false_negative_rate", "roc_auc_delta", "window_size", "seed", "status",
    ]
    win_fields = [
        "window_size", "false_activation_rate", "true_degradation_detection_power",
        "roc_auc_effect", "seed", "status",
    ]
    ...
    try:
        for w in requested:
            for s in range(int(args.seed_start), int(args.seed_start) + int(args.seeds)):
                ...
                try:
                    deg_records, win_records = run_one_seed_window(
                        cfg, s, w, args.experiment_id, benchmark, protocol
                    )
                    ...
                    fh_deg.flush()
                    fh_win.flush()
                    print(f"[b-mech-4 window={w} seed={s} done; "
                          f"{len(deg_records)} degradation rows]", flush=True)
                except Exception as exc:
                    print(f"[b-mech-4 window={w} seed={s} ERROR: {exc}]", flush=True)
                    for deg_type in DEGRADATION_TYPES:
                        w_deg.writerow({f: "" for f in deg_fields} | {
                            "degradation_type": deg_type, "window_size": w,
                            "seed": s, "status": f"error:{exc}"})
                    w_win.writerow({f: "" for f in win_fields} | {
                        "window_size": w, "seed": s, "status": f"error:{exc}"})
                    fh_deg.flush()
                    fh_win.flush()
    finally:
        fh_deg.close()
    ...
    print(f"[b-mech-4] {len(requested)} window sizes × {args.seeds} seeds complete. "
