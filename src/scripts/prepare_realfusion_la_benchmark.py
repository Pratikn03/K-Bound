...
def _load_unswnb15(path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    drop_cols = {"label", "attack_cat", "proto", "service", "state", "id", "srcip", "dstip", "sport", "dsport"}
    feature_cols = [c for c in df.columns if c not in drop_cols]
    X = pd.to_numeric(df[feature_cols].stack(), errors="coerce").unstack().fillna(0.0).values.astype(np.float32)
    return X, y
...
def _load_news_text(path: Path | None) -> tuple[np.ndarray, np.ndarray]:
    ...
    if path is not None and path.exists():
        ...
        normal_cats = ["comp.graphics", "comp.os.ms-windows.misc", "comp.sys.ibm.pc.hardware", "comp.sys.mac.hardware"]
        anomaly_cats = ["sci.med"]
        data_norm = fetch_20newsgroups(subset="all", categories=normal_cats, remove=("headers", "footers", "quotes"))
        data_anom = fetch_20newsgroups(subset="all", categories=anomaly_cats, remove=("headers", "footers", "quotes"))
...
def _label_align(
    ...
    if max_pos < n_pos:
        print(f"[warn] Requested {n_pos} positives but only {max_pos} available; " f"reducing n_composite accordingly.")
        n_pos = max_pos
    if max_neg < n_neg:
        print(f"[warn] Requested {n_neg} negatives but only {max_neg} available; " f"reducing n_composite accordingly.")
        n_neg = max_neg
...
def build_benchmark(
    creditcard_path: Path | None = None,
    unswnb15_path: Path | None = None,
    shoppers_path: Path | None = None,
    agnews_path: Path | None = None,
    ...
    if len(domain_data) < 4:
        print(
            f"[warn] Only {len(domain_data)}/4 domains available. " f"Benchmark will have fewer domains than the paper."
        )
...
def main() -> None:
    ...
    parser.add_argument("--creditcard", type=Path, default=None, help="Path to Kaggle creditcard.csv")
    parser.add_argument("--unswnb15", type=Path, default=None, help="Path to UNSW-NB15 CSV (training-set or full)")
    parser.add_argument("--shoppers", type=Path, default=None, help="Path to online_shoppers_intention.csv (UCI)")
    parser.add_argument(
        "--agnews",
        type=Path,
        default=None,
        help="Path to AG News CSV (optional; falls back to " "sklearn fetch_20newsgroups)",
    )
    parser.add_argument(
        "--output", type=Path, default=Path("data/real_fusion/realfusion_la.csv"), help="Output path for fusion CSV"
    )
    parser.add_argument(
        "--n-composite", type=int, default=8000, help="Number of composite samples to create (default: 8000)"
    )
    parser.add_argument(
        "--positive-rate", type=float, default=0.307, help="Target fraction of positive samples (default: 0.307)"
    )
    parser.add_argument(
        "--missing-rate",
        type=float,
        default=0.118,
        help="Fraction of domain observations to randomly drop " "(default: 0.118)",
    )
    parser.add_argument(
        "--embedding-dims", type=int, default=16, help="PCA embedding dimensions per domain (default: 16)"
    )
