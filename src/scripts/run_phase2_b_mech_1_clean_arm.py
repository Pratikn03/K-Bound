    ...
    for method, scores in (("static_attention", static_probs), ("rga_mean_gate_tau66", craf_probs)):
        frame = archive.build_frame(
            ...
            selected_head_or_comparator_status=(
                "RGA G0 mean-gate clean arm" if method == "rga_mean_gate_tau66" else "static reference clean arm"
            ),
            gate_mode="mean",
...
def main() -> int:
    ...
    for s in range(int(args.seed_start), int(args.seed_start) + int(args.seeds)):
        ...
        print(
            f"[b-mech-1 clean-arm seed={s} done; "
            f"static_clean_auc={r['static_test_auc']:.4f}  "
            f"rga_clean_auc={r['rga_test_auc']:.4f}]",
            flush=True,
        )
