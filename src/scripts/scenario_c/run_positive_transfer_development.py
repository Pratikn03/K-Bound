...
def _short_dataset(row: dict[str, Any]) -> str:
    ...
    if dataset_id == "mulsen_v2":
        return "MulSen"
    if dataset_id == "realiad_256_c1_c2_d13":
        return "Real-IAD"
...
def _official_status(row: dict[str, Any]) -> str:
    if bool(row.get("gate_e_positive_transfer_confirmed")):
        return "official pass"
    if str(row.get("status")) == "OFFICIAL_FAIL":
        return "official not confirmed"
    if str(row.get("holdout_status")) != "FRESH_OR_UNOPENED":
        return "not official"
    return "official unresolved"


def _load_official_result() -> dict[str, Any] | None:
    for path in (
        ROOT / "experiments/fusion/positive_transfer_confirmatory_result.json",
        ROOT / "elara_master_c/audits/positive_transfer_confirmatory_result.json",
    ):
        if not path.is_file():
            continue
        try:
...
def render_development_table(report: dict[str, Any]) -> str:
    """Render the D13 report as a manuscript-safe LaTeX table."""
    lines = [
    ...
    official = _load_official_result()
    if official is not None:
        dataset = _tex_escape(_short_dataset(official))
        rule = _tex_escape(str(official.get("selection", {}).get("selected_rule", "unknown")).replace("_", " "))
        sar = _fmt_effect(official.get("stats", {}).get("vs_sar", {}))
        cw = _fmt_effect(official.get("stats", {}).get("vs_cw", {}))
        status = _tex_escape(_official_status(official))
        lines.append(f"{dataset} & {rule} & {sar} & {cw} & {status} " + r"\\")
    lines.extend(
        [
            r"\bottomrule",
            r"\end{tabular}",
            "",
            "% Opened development evidence cannot set Gate E; official rows must still pass both SAR and CW endpoints.",
        ]
    )
    return "\n".join(lines)
